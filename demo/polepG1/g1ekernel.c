...
point3f FinalCP[2*GH_MAX_K][(G1H_FINALDEG+1)*(G1H_FINALDEG+1)];
...
char constrql2 = 0;
static char ConstrQueue2[8] = { -1, -1, -1, -1, -1, -1, -1, -1 };

/* ////////////////////////////////////////////////////////////////////////// */
static int G1OptionProc ( GHoleDomainf *domain, int query, int qn,
...
case G1HQUERY_CENTRAL_POINT:
    if ( CentralPointValid && swDisplayCentralPoint ) {
      ...
      *fdata = &central_point.x;
      return G1H_CENTRAL_POINT_GIVEN;
    }
    else
/*      return G1H_DEFAULT;*/
      return G1H_CENTRAL_POINT_ALT;

case G1HQUERY_CENTRAL_DERIVATIVES1:
    if ( CentralPointValid && swUseDerivatives1 ) {
      *ndata = 2*hole_k;
      *idata = NULL;
      *fdata = &domcder[0].x;
      return G1H_CENTRAL_DERIVATIVES1_GIVEN;
    }
    else
      return G1H_DEFAULT;
...
void RecreateDomain ()
{
  ...
  g1h_SetOptionProcf ( domain, G1OptionProc );
  if ( g1h_ComputeBasisf ( domain ) )
    if ( !CentralPointValid || !swUseDerivatives1 ) {
      g1h_ExtractCentralPointf ( domain, &central_point, domcder );
...
{
  if ( !FormMatrixValid )
    FormMatrixValid = g1h_ComputeFormMatrixf ( domain );
...
boolean UpdateExtFormMatrix ()
{
  if ( !ExtFormMatrixValid )
    ExtFormMatrixValid = g1h_ComputeExtFormMatrixf ( domain );
  return ExtFormMatrixValid;
} /*UpdateExtFormMatrix*/

void OutFinalPatch ( int n, int m, const float *cp, void *usrptr )
{
  if ( nfinal < 2*GH_MAX_K && n == G1H_FINALDEG && m == G1H_FINALDEG ) {
  ...
  if ( swCoonsPatches ) {
    if ( FormMatrixValid ) {
      ...
      if ( !swConstraintsOn || !nconstr || !ConstraintsValid ) {
        FinalSurfValid = g1h_FillHolef ( domain, 3, (float*)surfcp,
        ...
        if ( !swNormalConstr ) {
          ...
          FinalSurfValid = g1h_FillHoleConstrf ( domain, 3, (float*)surfcp,
          ...
          FinalSurfValid = g1h_FillHoleAltConstrf ( domain, 3, (float*)surfcp,
    ...
    if ( ExtFormMatrixValid ) {
      nfinal = 0;
      if ( !swConstraintsOn || !nconstr || !ConstraintsValid ) {
        FinalSurfValid = g1h_ExtFillHolef ( domain, 3, (float*)surfcp,
                                            NULL, NULL, OutFinalPatch );
        if ( !ConstraintsValid )
          GetConstraintsHandle ();
      }
        ...
        if ( !swNormalConstr ) {
          ...
          if ( !rhs )
            return false;
          SetupConstraintsRHS ( rhs );
          FinalSurfValid = g1h_ExtFillHoleConstrf ( domain, 3, (float*)surfcp,
                                    nconstr, (float*)rhs, NULL, NULL,
                                    OutFinalPatch );
        }
        else {
          ...
          if ( !rhsf )
            return false;
          SetupNConstraintsRHS ( rhsf );
          FinalSurfValid = g1h_ExtFillHoleAltConstrf ( domain, 3, (float*)surfcp,
                                  nconstr, rhsf, NULL, NULL, OutFinalPatch );
        }
        pkv_SetScratchMemTop ( sp );
      }
...
boolean UpdateNLFinalSurf ()
{
  ...
  if ( swCoonsPatches ) {
    if ( !swConstraintsOn || !nconstr )
      NLFinalSurfValid = g1h_NLFillHolef ( domain, surfcp, NULL,
      ...
      if ( !swNormalConstr ) {
        if ( !ConstrMatrixValid )
          if ( !SetupConstraintsMatrix () )
            return false;
        rhs = (point3f*)pkv_GetScratchMem ( nconstr*sizeof(point3f) );
        if ( !rhs )
          return false;
        SetupConstraintsRHS ( rhs );
        NLFinalSurfValid =
          g1h_NLFillHoleConstrf ( domain, surfcp, nconstr, rhs,
        ...
        if ( !rhsf )
          return false;
        SetupNConstraintsRHS ( rhsf );
        NLFinalSurfValid = g1h_NLFillHoleAltConstrf ( domain, surfcp, nconstr,
                                      rhsf, NULL, NULL, (outfunc3)OutFinalPatch  );
      }
    }
  }
  else {
    if ( !swConstraintsOn || !nconstr )
      NLFinalSurfValid = g1h_NLExtFillHolef ( domain, surfcp, NULL,
                                              NULL, (outfunc3)OutFinalPatch );
    else {
      if ( !swNormalConstr ) {
        if ( !ExtConstrMatrixValid )
        ...
        SetupConstraintsRHS ( rhs );
        NLFinalSurfValid =
          g1h_NLExtFillHoleConstrf ( domain, surfcp, nconstr, rhs,
                                     NULL, NULL, (outfunc3)OutFinalPatch );
      }
      else {
        if ( !ExtNConstrMatrixValid )
          ...
          if ( !rhsf )
            return false;
          SetupNConstraintsRHS ( rhsf );
          NLFinalSurfValid = g1h_NLExtFillHoleAltConstrf ( domain, surfcp,
...
void ResetCPos ()
{
  ...
  SetPoint3f ( &CPos.g_centre, 0.5*(RefBBox.x0+RefBBox.x1),
           0.5*(RefBBox.y0+RefBBox.y1), 0.5*(RefBBox.z0+RefBBox.z0) );
  CPos.c_centre = CPos.g_centre;
  bx = RefBBox.x0-RefBBox.x1;
  by = RefBBox.y0-RefBBox.y1;
  bz = RefBBox.z0-RefBBox.z0;
...
{
  CameraInitFramef ( &CPos, false, false, edr->w, edr->h, edr->x, edr->y, 1.0, 4 );
  if ( reset )
    ResetCPos ();
  else
...
void SetupParProj ( ed_rect *edr )
{
  ...
  for ( i = 0; i < 3; i++ ) {
    CameraInitFramef ( &PPos[i], true, false,
                       edr[i].w, edr[i].h, edr[i].x, edr[i].y, aspect, 4 );
...
void RedrawDomainCurves ()
{
  ...
  colour2 = c_dk_grey;
  dens1 = dens2 = 1;
  g1h_DrawDiPatchesf ( domain, DrawDomBezPatch );
} /*RedrawDomainCurves*/
...
void RedrawDomainAuxPatches ()
{
  ...
  colour2 = c_red;
  dens1 = dens2 = 8;
  g1h_DrawDomAuxPatchesf ( domain, DrawDomAuxPatch );
...
void RedrawDomainNumbers ()
{
  ...
  pointnum = 0;
  g1h_DrawDomAuxPatchesf ( domain, DrawDomNumbers );
} /*RedrawDomainNumbers*/
...
*/
  ...
  g1h_ComputeNLNormalf ( domain, surfcp, &nv );
...
{
#define NFCP ((G1H_FINALDEG+1)*(G1H_FINALDEG+1))
  void    *sp;
  point3f *cp, *fcp, v1, v2;
  int     i, j, k, l, m, n;

  sp = pkv_GetScratchMemTop ();
  if ( FinalSurfValid || NLFinalSurfValid ) {
    ...
    for ( i = 0; i < hole_k; i++ ) {
      pkv_Selectc ( 5, sizeof(point3f), (G1H_FINALDEG+1)*sizeof(point3f),
      ...
      for ( j = 1, k = l = G1H_FINALDEG, m = n = 4;
            j <= 4;
            j++, l *= --k, n *= --m ) {
...
boolean SetupConstraintsMatrix ()
{
  ...
  sp = pkv_GetScratchMemTop ();
  if ( !(dimV0 = g1h_V0SpaceDimf ( domain )) )
    goto failure;

  constr = pkv_GetScratchMemf ( nconstr*dimV0 );
  drval  = pkv_GetScratchMemf ( 3*dimV0 );
  if ( !constr || !drval )
    goto failure;
  ...
  s   = 1;
  for ( i = 0; i < hole_k; i++ ) {
    if ( !g1h_GetBPDerivativesf ( domain, i, drval ) )
      goto failure;
    if ( i == 0 && swConstraint[0] ) {
      pkv_Selectf ( dimV0, 1, 3, 1, drval, constr );
      ncn = 1;
    }
    for ( j = 1;  j <= 4;  j++, s++ )
      if ( swConstraint[s] ) {
        pkv_Selectf ( dimV0, 1, 3, 1, &drval[j], &constr[ncn*dimV0] );
        ncn++;
      }
  }
  pkv_FreeScratchMemf ( 3*dimV0 );
  if ( !g1h_SetConstraintMatrixf ( domain, nconstr, constr ) )
    goto failure;
...
boolean SetupExtConstraintsMatrix ()
{
  ...
  if ( !(dimV0 = g1h_V0SpaceDimf ( domain )) )
    goto failure;
  dimExtV0 = g1h_ExtV0SpaceDimf ( domain );
  dd = dimExtV0-dimV0;
  ...
  for ( i = 0; i < hole_k; i++ ) {
    if ( !g1h_GetBPDerivativesf ( domain, i, drval ) )
      goto failure;
    if ( i == 0 && swConstraint[0] ) {
      pkv_Selectf ( dimV0, 1, 3, 1, drval, &constr[dd] );
      ncn = 1;
    }
    for ( j = 1;  j <= 4;  j++, s++ )
      if ( swConstraint[s] ) {
        pkv_Selectf ( dimV0, 1, 3, 1, &drval[j], &constr[ncn*dimExtV0+dd] );
        ncn++;
      }
  }
  pkv_FreeScratchMemf ( 3*dimV0 );
  if ( !g1h_SetExtConstraintMatrixf ( domain, nconstr, constr ) )
    goto failure;
...
boolean SetupNConstraintsMatrix ()
{
  ...
  for ( i = 0; i < hole_k; i++ ) {
    ...
    for ( j = 1;  j <= 4;  j++, s++ )
      if ( swConstraint[s] ) {
        pkv_Selectf ( dimV0, 1, 3, 1, &drval[j], &constr[3*ncn*dimV0] );
        ncn++;
      }
  }
  pkv_FreeScratchMemf ( 3*dimV0 );
  ...
  if ( !g1h_SetAltConstraintMatrixf ( domain, 3, nconstr, constr ) )
    goto failure;
...
boolean SetupExtNConstraintsMatrix ()
{
  ...
  for ( i = 0; i < hole_k; i++ ) {
    ...
    for ( j = 1;  j <= 4;  j++, s++ )
      if ( swConstraint[s] ) {
        pkv_Selectf ( dimV0, 1, 3, 1, &drval[j], &constr[3*ncn*dimExtV0+dd] );
        ncn++;
      }
  }
  pkv_FreeScratchMemf ( 3*dimV0 );
  ...
  if ( !g1h_SetExtAltConstraintMatrixf ( domain, 3, nconstr, constr ) )
    goto failure;
...
void SwitchAConstraint ( int cno )
{
  ...
  if ( swZeroDer ) {
    ...
    case 2:
        if ( !(hole_k % 2) )
          InsertSWQ ( ConstrQueue2, hole_k-1, &constrql2, nc, 2 );
        break;
   default:
        break;
      }
