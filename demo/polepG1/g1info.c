...
static void WritePartitionInfo ()
{
  ...
  if ( partition ) {
    g1h_ExtractPartitionf ( domain, &hole_k, &hole_m, partition,
...
{
  g1h_DrawMatricesf ( domain, DrawMatrix );
} /*WriteMatrixInfo*/
...
static void WriteFuncValue ()
{
  ...
  if ( acoeff ) {
    g1h_FillHolef ( domain, 3, (float*)surfcp, acoeff, NULL, EmptyOutput );
    fval = g1h_FunctionalValuef ( domain, 3, (float*)surfcp, acoeff );
    printf ( "F(u) = %f\n", fval );
  }
  pkv_SetScratchMemTop ( sp );
} /*WriteFuncValue*/
...
static void WriteConstrInfo ()
{
  ...
  if ( acoeff ) {
    if ( !swNormalConstr ) {
      ...
      g1h_FillHoleConstrf ( domain, 3, (float*)surfcp,
      ...
      g1h_FillHoleAltConstrf ( domain, 3, (float*)surfcp,
...
{
  g1h_DrawExtMatricesf ( domain, DrawExtMatrix );
} /*WriteExtMatrixInfo*/
...
static void WriteExtFuncValue ()
{
  ...
  if ( acoeff ) {
    g1h_ExtFillHolef ( domain, 3, (float*)surfcp, acoeff, NULL, EmptyOutput );
    fval = g1h_ExtFunctionalValuef ( domain, 3, (float*)surfcp, acoeff );
    printf ( "F(u) = %f\n", fval );
  }
  pkv_SetScratchMemTop ( sp );
} /*WriteExtFuncValue*/
  ...
  if ( acoeff ) {
    if ( !swNormalConstr ) {
      if ( !(rhs = pkv_GetScratchMem ( nconstr*sizeof(point3f) )) )
        goto wayout;
      SetupConstraintsRHS ( rhs );
      g1h_ExtFillHoleConstrf ( domain, 3, (float*)surfcp,
                               nconstr, (float*)rhs, acoeff, NULL, EmptyOutput );
    }
    else {
      if ( !(rhsf = pkv_GetScratchMemf ( nconstr )) )
        goto wayout;
      SetupNConstraintsRHS ( rhsf );
      g1h_ExtFillHoleAltConstrf ( domain, 3, (float*)surfcp,
                                  nconstr, rhsf, acoeff, NULL, EmptyOutput );
    }
    fval = g1h_ExtFunctionalValuef ( domain, 3, (float*)surfcp, acoeff );
    printf ( "%d constraints, F(u) = %f\n", nconstr, fval );
