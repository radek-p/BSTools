...
void InitSide10Menu_BSm ( void )
{
  ...
  w = xge_NewButton ( win1, w, btnM1BSM_TRIANG_FACETS, 79, 19, 0, 140,
                      txtTriangulate );
  w = xge_NewIntWidget ( win1, w, intwM1BSM_SIMPLIFY_X, 36, 19, 0, 160,
                         10, 100, &intw_bsm_simplify_x, txtX, &bsm_simplify_xyz[0] );
  w = xge_NewIntWidget ( win1, w, intwM1BSM_SIMPLIFY_Y, 36, 19, 37, 160,
                         10, 100, &intw_bsm_simplify_y, txtY, &bsm_simplify_xyz[1] );
  w = xge_NewIntWidget ( win1, w, intwM1BSM_SIMPLIFY_Z, 36, 19, 74, 160,
...
case xgemsg_BUTTON_COMMAND:
  ...
  case btnM1BSM_SIMPLIFY_MESH:
      if ( GeomObjectBSplineMeshSimplify ( obj, bsm_simplify_xyz ) ) {
        bsm_sw_blending = obj->blending;
        SetupBSplineMeshVEFnum ( obj );
        rendered_picture = false;
        xge_RedrawAll ();
...
case xgemsg_INT_WIDGET_COMMAND:
  ...
  case intwM1BSM_DEG:
      ...
      return 1;
  case intwM1BSM_SIMPLIFY_X:
      bsm_simplify_xyz[0] = key;
      return 1;
  case intwM1BSM_SIMPLIFY_Y:
      bsm_simplify_xyz[1] = key;
      return 1;
