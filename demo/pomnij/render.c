...
static int npatches;
static RBezPatchTreefp *patch_tab = NULL;
...
float  render_dfsf = 10.0;
float  render_cfrange[2] = {0.0,1.0};

static float minsf, maxsf;

static boolean (*dRShapeFunc)( int n, int m, const point4f *cp, float u, float v,
                       point3f *p, vector3f *nv, vector3f *vv );
static float (*cRShapeFunc)( int n, int m, const point4f *cp, float u, float v,
                     point3f *p, vector3f *nv, vector3f *vv );
...
boolean dRShapeFunc0 ( int n, int m, const point4f *cp, float u, float v,
                       point3f *p, vector3f *nv, vector3f *vv )
{
  return true;
} /*dRShapeFunc0*/

boolean dRHighlightLines ( int n, int m, const point4f *cp, float u, float v,
                            point3f *p, vector3f *nv, vector3f *vv )
{
  vector3f dp;
  double   s;
  ...
  s = render_dfsf*det3f ( nv, &dp, &rv2 ) / det3f ( nv, &rv1, &rv2 );
  if ( s < 0.0 )
    s = 1.0-s;
  return ((int)s) & 0x01;
} /*dRHighlightLines*/

boolean dRReflectionLines ( int n, int m, const point4f *cp, float u, float v,
                            point3f *p, vector3f *nv, vector3f *vv )
{
  vector3f r, dp;
  double   s;
  ...
  SubtractPoints3f ( p, &rp0, &dp );
  s = render_dfsf*det3f ( &r, &dp, &rv2 ) / det3f ( &r, &rv1, &rv2 );
  if ( s < 0.0 )
    s = 1.0-s;
  return !(((int)s) & 0x01);
} /*dRReflectionLines*/

boolean dRCrossSections ( int n, int m, const point4f *cp, float u, float v,
                          point3f *p, vector3f *nv, vector3f *vv )
{
  double s;

  s = render_dfsf*DotProduct3f ( p, &sectiondir );
  if ( s < 0.0 )
    s = 1.0-s;
  return !(((int)s) & 0x01);
} /*dRCrossSections*/

boolean dRLambertLight ( int n, int m, const point4f *cp, float u, float v,
                         point3f *p, vector3f *nv, vector3f *vv )
{
  double s;

  s = render_dfsf*DotProduct3f ( nv, &lightdir[0] );
  if ( s < 0.0 )
    s = 1.0-s;
  return !(((int)s) & 0x01);
} /*dRLambertLight*/

boolean dRMeanCurvature ( int n, int m, const point4f *cp, float u, float v,
                          point3f *p, vector3f *nv, vector3f *vv )
{
  float gauss, mean, s;

  mbs_GMCurvaturesBP3Rf ( n, m, cp, u, v, &gauss, &mean );
  s = render_dfsf*mean;
  if ( s < 0.0 )
    s = 1.0-s;
  return !(((int)s) & 0x01);
...
boolean dRGaussianCurvature ( int n, int m, const point4f *cp, float u, float v,
                              point3f *p, vector3f *nv, vector3f *vv )
{
  float gauss, mean, s;
...
boolean dRParamQuality ( int n, int m, const point4f *cp, float u, float v,
                         point3f *p, vector3f *nv, vector3f *vv )
{
  point3f  pp;
  vector3f pu, pv;
  float    g11, g12, g22, pq, s;

  mbs_BCHornerDerP3Rf ( n, m, cp, u, v, &pp, &pu, &pv );
  g11 = DotProduct3f ( &pu, &pu );
  g12 = DotProduct3f ( &pu, &pv );
  g22 = DotProduct3f ( &pv, &pv );
  pq = ((g11-g22)*(g11-g22) + 4.0*g12*g12)/((g11+g22)*(g11+g22));
  s = render_dfsf*pq;
  if ( s < 0.0 )
    s = 1.0-s;
  return !(((int)s) & 0x01);
...
float cRShapeFunc0 ( int n, int m, const point4f *cp, float u, float v,
                     point3f *p, vector3f *nv, vector3f *vv )
{
  return 0.0;
} /*cRShapeFunc0*/

float cRHighlightLines ( int n, int m, const point4f *cp, float u, float v,
                          point3f *p, vector3f *nv, vector3f *vv )
{
  vector3f dp;
  double   s;
...
float cRReflectionLines ( int n, int m, const point4f *cp, float u, float v,
                           point3f *p, vector3f *nv, vector3f *vv )
{
  vector3f r, dp;
  double   s;
...
float cRCrossSections ( int n, int m, const point4f *cp, float u, float v,
                        point3f *p, vector3f *nv, vector3f *vv )
{
  double s;
...
float cRLambertLight ( int n, int m, const point4f *cp, float u, float v,
                       point3f *p, vector3f *nv, vector3f *vv )
{
  double s;
...
float cRMeanCurvature ( int n, int m, const point4f *cp, float u, float v,
                        point3f *p, vector3f *nv, vector3f *vv )
{
  float gauss, mean;
...
float cRGaussianCurvature ( int n, int m, const point4f *cp, float u, float v,
                            point3f *p, vector3f *nv, vector3f *vv )
{
  float gauss, mean;
...
float cRParamQuality ( int n, int m, const point4f *cp, float u, float v,
                       point3f *p, vector3f *nv, vector3f *vv )
{
  point3f  pp;
  vector3f pu, pv;
  ...
  g12 = DotProduct3f ( &pu, &pv );
  g22 = DotProduct3f ( &pv, &pv );
  return ((g11-g22)*(g11-g22) + 4.0*g12*g12)/((g11+g22)*(g11+g22));
} /*cRParamQuality*/
...
#define DENS 20
  ...
  for ( i = 0; i < npatches; i++ ) {
    ...
    if ( !ok ) {
      mbs_BCHornerNvP3Rf ( n, m, cp, 0.0, 0.0, &p, &nv );
      SubtractPoints3f ( &p, &_CPos.position, &vv );
      NormalizeVector3f ( &vv );
      fmin = fmax = cRShapeFunc ( n, m, cp, 0.0, 0.0, &p, &nv, &vv );
      ok = true;
    }
    for ( j = 0; j <= DENS; j++ ) {
      ...
      for ( k = 0; k <= DENS; k++ ) {
        ...
        mbs_BCHornerNvP3Rf ( n, m, cp, u, v, &p, &nv );
        SubtractPoints3f ( &p, &_CPos.position, &vv ); 
        NormalizeVector3f ( &vv );
        f = cRShapeFunc ( n, m, cp, u, v, &p, &nv, &vv );
        if ( f < fmin )
          fmin = f;
        else if ( f > fmax )
          fmax = f;
  ...
  minsf = fmin + (fmax-fmin)*render_cfrange[0];
  maxsf = fmin + (fmax-fmin)*render_cfrange[1];
  printf ( "Shape function min = %f, max = %f\n", minsf, maxsf );
  if ( cRShapeFunc == cRParamQuality )
    minsf = 0.0;
  if ( minsf >= maxsf ) {
    minsf -= 2.0;
    maxsf += 3.0;
...
boolean RendEnterBezPatch3Rf ( int n, int m, const point4f *cp )
{
  if ( patch_tab && npatches < R_MAXPATCHES ) {
    patch_tab[npatches] = rbez_NewRBezPatchTreef ( 0, n, m, 0.0, 1.0, 0.0, 1.0, cp );
...
boolean RendEnterBezPatch3Rd ( int n, int m, const point4d *cp )
{
  ...
  if ( patch_tab && npatches < R_MAXPATCHES ) {
    size = (n+1)*(m+1);
    cpf = pkv_GetScratchMem ( size*sizeof(point4f) );
    if ( !cpf )
      goto failure;
    pkv_Selectdf ( 1, 4*size, 1, 1, (double*)cp, (float*)cpf );
    patch_tab[npatches] = rbez_NewRBezPatchTreef ( 0, n, m, 0.0, 1.0, 0.0, 1.0, cpf );
    if ( patch_tab[npatches] ) {
      npatches ++;
      pkv_SetScratchMemTop ( sp );
      return true;
    ...
    if ( b && c ) {
      ...
      for ( i = 0; i < ku; i++ )
        for ( j = 0; j < kv; j++ ) {
          ...
          if ( !RendEnterBezPatch3Rd ( n, m, (point4d*)c ) )
            goto failure;
        }
      pkv_SetScratchMemTop ( sp );
      return true;
...
boolean RendReset ( void )
{
  ...
  if ( RenderingIsOn ) {
    ...
    root = NULL;
    for ( i = 0; i < npatches; i++ ) {
      rbez_DestroyRBezPatchTreef ( patch_tab[i] );
      patch_tab[i] = NULL;
    }
#ifdef COPY_POSTSCRIPT
    DumpPostscriptPicture ();
                        ...
                        int *ninters, RayObjectIntersf *inters, int *k )
{
  ...
  if ( (_k = node->k) >= 0 ) {  /* it is a leaf, i.e. a Bezier patch */
    ...
    if ( rbez_FindRayRBezPatchIntersf (
...
void GetTexColour ( int n, int m, const point4f *cp, float u, float v,
                    point3f *p, vector3f *nv, vector3f *vv,
                    vector3f *texcolour )
{
  float t;
