...
int main ()
{
  ...
  if ( (domain = gh_CreateDomainf ( HOLE_K, knots, Dompt )) ) {
    if ( g2h_ComputeBasisf ( domain ) ) {
      ...
      nfunc_a = g2h_V0SpaceDimf ( domain );
      nfunc_c = HOLE_K*DBDIM;
      bfder = pkv_GetScratchMemf ( nfunc_a*HOLE_K*5 );
      if ( !bfder )
        goto failure;
      if ( !g2h_GetBPDerivativesf ( domain, 0, bfder ) )
        goto failure;
      cmat = pkv_GetScratchMemf ( 2*(nfunc_a+nfunc_c) );
      if ( !cmat )
        goto failure;
      pkv_Selectf ( nfunc_a, 1, 5, 1, bfder, cmat );
      pkv_Selectf ( nfunc_a, 1, 5, 1, &bfder[1], &cmat[nfunc_a] );
      if ( !g2h_SetConstraintMatrixf ( domain, 2, cmat ) )
        goto failure;
      SetPoint3f ( &cnt[0], 0.25, 0.25, -0.8 );
      SetPoint3f ( &cnt[1], 1.0, 0.0, 1.0 );
      if ( !g2h_FillHoleConstrf ( domain, 3, (float*)Bpt,
                                  2, &cnt[0].x, acoeff, NULL, OutPatch ) )
        goto failure;

      memset ( cmat, 0, 2*(nfunc_a+nfunc_c)*sizeof(float) );
      pkv_Selectf ( nfunc_a, 1, 5, 1, bfder, &cmat[nfunc_c] );
      pkv_Selectf ( nfunc_a, 1, 5, 1, &bfder[1], &cmat[nfunc_a+nfunc_c+nfunc_c] );
      if ( !g2h_SetExtConstraintMatrixf ( domain, 2, cmat ) )
        goto failure;
      if ( !g2h_ExtFillHoleConstrf ( domain, 3, (float*)Bpt,
                                     2, &cnt[0].x, bcoeff, NULL, OutPatch ) )
        goto failure;

      times ( &stop );
      fval = g2h_FunctionalValuef ( domain, 3, (float*)Bpt, acoeff );
      efval = g2h_ExtFunctionalValuef ( domain, 3, (float*)Bpt, bcoeff );
