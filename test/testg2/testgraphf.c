...
float ComputeLaplacian ( int n, int m, const point2f *xycp, const float *zcp,
                         float u, float v )
{
  vector2f xy, xyu, xyv, xyuu, xyuv, xyvv;
  vector2f gx, gy, gxx, gxy, gyy;
  float    xu, xv, xuu, xuv, xvv, yu, yv, yuu, yuv, yvv,
           z, zu, zv, zuu, zuv, zvv;
  float    lap, jac, a[9], b[6], c[6], trd[5];

  mbs_BCHornerDer2P1f ( n, m, zcp, u, v, &z, &zu, &zv, &zuu, &zuv, &zvv );
  mbs_BCHornerDer2P2f ( n, m, xycp, u, v,
  ...
  pkn_multiSolveRLSQf ( 2, 2, a, 2, 2, b, 2, c );
  gx.x = c[0];  gx.y = c[1];
  gy.x = c[2];  gy.y = c[3];
...
/* here c[0]*zu + c[1]*zv = zx;  c[2]*zu + c[3]*zv = zy */
  ...
  pkn_multiSolveRLSQf ( 3, 3, a, 2, 2, b, 2, c );
  gxx.x = c[0];  gxx.y = c[1];
  gxy.x = c[2];  gxy.y = c[3];
  gyy.x = c[4];  gyy.y = c[5];
                            ...
                            float u, float v, float *jac, vector2f *lgr )
{
  ...
  FILE *f;
#endif
  vector2f xy, xyu, xyv, xyuu, xyuv, xyvv, xyuuu, xyuuv, xyuvv, xyvvv;
  vector2f gx, gy, gxx, gxy, gyy, gxxx, gxxy, gxyy, gyyy;
  float    xu, xv, xuu, xuv, xvv, xuuu, xuuv, xuvv, xvvv,
           yu, yv, yuu, yuv, yvv, yuuu, yuuv, yuvv, yvvv,
           z, zu, zv, zuu, zuv, zvv, zuuu, zuuv, zuvv, zvvv;
  float    a[16], b[8], c[8], trd[18];
#ifdef DUMP
  f = fopen ( "lapgr.dat", "w+" );
  fprintf ( f, "n=%d, m=%d, u=%f, v=%f\n", n, m, u, v );
#endif
  mbs_BCHornerDer3P1f ( n, m, zcp, u, v, &z, &zu, &zv, &zuu, &zuv, &zvv,
                       &zuuu, &zuuv, &zuvv, &zvvv );
  mbs_BCHornerDer3P2f ( n, m, (float*)xycp, u, v,
                       (float*)&xy, (float*)&xyu, (float*)&xyv,
                       (float*)&xyuu, (float*)&xyuv, (float*)&xyvv,
                       (float*)&xyuuu, (float*)&xyuuv, (float*)&xyuvv,
                       (float*)&xyvvv );
#ifdef DUMP
...
*/
  ...
  pkn_multiSolveRLSQf ( 4, 4, a, 2, 2, b, 2, c );
                         ...
                         point3f *gp )
{
  ...
  xycp = (point2f*)pkv_GetScratchMem ( (n+1)*(m+1)*sizeof(point2f) );
  zcp  = pkv_GetScratchMemf ( (n+1)*(m+1) );
  if ( !xycp || !zcp )
    exit ( 1 );

  pkv_Selectf ( (n+1)*(m+1), 2, 3, 2, cp, xycp );
  pkv_Selectf ( (n+1)*(m+1), 1, 3, 1, &cp[0].z, zcp );

  for ( i = 0; i <= dd; i++ ) {
    v = (float)i/dd;
    mbs_BCHornerP2f ( n, m, (float*)xycp, u, v, &gp[i].x );
