...
void DrawAuxBasisFuncBSPatch ( int fn, double x, double y )
{
  ...
  ps_Write_Command ( s );
  ps_Set_Gray ( 0.5 );
  g2h_DrawSplBasAuxPatchesd ( domain, fn, DrawAuxBSPatch );
  ps_GRestore ();
...
void DrawBasisFuncBSPatch ( int fn, double x, double y )
{
  ...
  ps_Set_Gray ( 0.5 );
  g2h_DrawSplBasFunctiond ( domain, fn, DrawBSPatch );
  if ( fn >= nfunc_a+nfunc_b+nfunc_c ) {
    cnt = -((fn-(nfunc_a+nfunc_b+nfunc_c)) / (nfunc_d/HOLE_K));
    ps_Set_RGB ( 1.0, 0.0, 0.0 );
    g2h_DrawSplBasAuxPatchesd ( domain, fn, AltDrawAuxBSPatch );
  }
  ps_GRestore ();
...
void DrawBasisFuncBSPatchL ( int fn, double x, double y )
{
  ...
  ps_Set_Gray ( 0.5 );
  maxlap = 0.0;
  g2h_DrawSplBasFunctiond ( domain, fn, DrawBSPatchL1 );
  scf = 0.5/maxlap;
  g2h_DrawSplBasFunctiond ( domain, fn, DrawBSPatchL2 );
  ps_GRestore ();
...
void DrawSplMatrix ( int k, int r, int s, int t, double *A, double *B )
{
  ...
  if ( buf ) {
    ps_GSave ();
    ps_Write_Command ( "100 100 translate 4 dup scale" );
...
{
  ps_OpenFile ( fn3, 600 );
  g2h_DrawSplMatricesd ( domain, DrawSplMatrix );
  ps_CloseFile ();
  printf ( "%s\n", fn3 );
} /*MakePicture3*/
...
void AllocPatchStorage ( void )
{
  ...
  for ( i = 0; i < HOLE_K; i++ ) {
    knu1[i] = malloc ( (n+G2H_FINALDEG+1)*sizeof(double) );
    knv1[i] = malloc ( (n+G2H_FINALDEG+1)*sizeof(double) );
    scp1[i] = malloc ( n*n*sizeof(point3d) );
    knu2[i] = malloc ( (n+G2H_FINALDEG+1)*sizeof(double) );
    knv2[i] = malloc ( (n+G2H_FINALDEG+1)*sizeof(double) );
...
double CompIntegral ( void )
{
  ...
  vector2d lgr1, lgr2;
  double    jac1, jac2, gx, gy, gxx, gxy, gyy, gxxx, gxxy, gxyy, gyyy;
  ...
  ap1 = pkv_GetScratchMem ( 4*sz*sizeof(point3d) );
  ap2 = pkv_GetScratchMem ( 4*sz*sizeof(point3d) );
  if ( !ap1 || !ap2 )
    exit ( 1 );
  sss = 0.0;
  for ( i = 0; i < HOLE_K; i++ ) {
    if ( IsZnonZero ( sdegu1[i], lknu1[i], knu1[i],
      ...
      for ( j = 0; j < QUAD_FACTOR*(NK+1); j++ ) {
        ...
        mbs_multideBoorDer3d ( sdegu1[i], lknu1[i], knu1[i], 1, 3*d1, 0,
            (double*)scp1[i], u, &ap1[0].x, &ap1[d1].x, &ap1[2*d1].x, &ap1[3*d1].x );
        mbs_multideBoorDer3d ( sdegu2[i], lknu2[i], knu2[i], 1, 3*d2, 0,
            (double*)scp2[i], u, &ap2[0].x, &ap2[d2].x, &ap2[2*d2].x, &ap2[3*d2].x );
        s = 0.0;
        for ( k = 0; k < QUAD_FACTOR*(NK+1); k++ ) {
          v = (double)(k+k+1)/(double)(2*QUAD_FACTOR*(NK+1));
          mbs_multideBoorDer3d ( sdegv1[i], lknv1[i], knv1[i], 4, 3, 3*d1,
              (double*)ap1, v, &p1[0].x, &p1[4].x, &p1[8].x, &p1[12].x );
          mbs_multideBoorDer3d ( sdegv2[i], lknv2[i], knv2[i], 4, 3, 3*d2,
              (double*)ap2, v, &p2[0].x, &p2[4].x, &p2[8].x, &p2[12].x );
          pkn_Comp2iDerivatives3d ( p1[1].x, p1[1].y, p1[4].x, p1[4].y,
          ...
          pkn_Comp2iDerivatives3d ( p2[1].x, p2[1].y, p2[4].x, p2[4].y,
...
/* a better chance of being programmed without bugs */
  ...
  for ( i = 0; i < nfunc_a+nfunc_c+nfunc_d; i++ ) {
    ...
    cnt = 0;
    g2h_DrawSplBasFunctiond ( domain, fni, SavePatch1 );
    for ( j = 0; j <= i; j++ ) {
    if ( j < nfunc_c+nfunc_d ) fnj = nfunc_a+nfunc_b+j;
      ...
      if ( k >= 0 ) {  /* this element belongs to a nonzero block of A */
        cnt = 0;
        g2h_DrawSplBasFunctiond ( domain, fnj, SavePatch2 );
        AAmat[k] = CompIntegral ();
      }
    }
    for ( j = 0; j < nfunc_b; j++ ) {
      cnt = 0;
      g2h_DrawSplBasFunctiond ( domain, nfunc_a+j, SavePatch2 );
...
{
  AllocPatchStorage ();
  g2h_DrawSplMatricesd ( domain, SaveMatrices );
  WriteBlockAddr ();
