...
char fn[] = "g2hspld.ps";
char datfn1[] = "g2hspld.dat";
char datfn2[] = "g2hextd.dat";
char datfn3[] = "g2hd.dat";
...
int main ( void )
{
  ...
  if ( (domain = gh_CreateDomaind ( HOLE_K, knots, Dompt )) ) {
    if ( g2h_ComputeSplBasisd ( domain, NK, M1, M2 ) ) {
      nfbpatches = 0;
      if ( !g2h_FillHoled ( domain, 3, (double*)Bpt, NULL, NULL, OutputBezPatch ) )
        exit ( 1 );
      if ( !g2h_ExtFillHoled ( domain, 3, (double*)Bpt, NULL, NULL, OutputBezPatch ) )
        exit ( 1 );
      nfpatches = 0;
      if ( g2h_SplFillHoled ( domain, 3, (double*)Bpt, NULL, NULL, OutputSplPatch ) ) {
        times ( &stop );
