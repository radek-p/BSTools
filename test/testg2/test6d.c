...
int main ()
{
  ...
  if ( (domain = gh_CreateDomaind ( HOLE_K, knots, Dompt )) ) {
    if ( g2h_ComputeBasisd ( domain ) ) {
      ...
      bfder = pkv_GetScratchMemd ( nfunc_a*HOLE_K*5 );
      if ( !bfder )
        goto failure;
      ...
      pkv_Selectd ( nfunc_a, 1, 5, 1, &bfder[1], &cmat[nfunc_a] );
      if ( !g2h_SetConstraintMatrixd ( domain, 2, cmat ) )
        goto failure;
      SetPoint3d ( &cnt[0], 0.15, 0.05, -0.6 );
      SetPoint3d ( &cnt[1], 1.0, 0.0, 0.5 );
      if ( !g2h_NLFillHoleConstrd ( domain, Bpt,
                                    2, cnt, acoeff, NULL, OutPatch ) )
        goto failure;

      memset ( cmat, 0, 2*(nfunc_a+nfunc_c)*sizeof(double) );
      pkv_Selectd ( nfunc_a, 1, 5, 1, bfder, &cmat[nfunc_c] );
      pkv_Selectd ( nfunc_a, 1, 5, 1, &bfder[1], &cmat[nfunc_a+nfunc_c+nfunc_c] );
      if ( !g2h_SetExtConstraintMatrixd ( domain, 2, cmat ) )
        goto failure;
      if ( !g2h_NLExtFillHoleConstrd ( domain, Bpt,
                                       2, cnt, bcoeff, NULL, OutPatch ) )
        goto failure;

      times ( &stop );
      fval = g2h_FunctionalValued ( domain, 3, (double*)Bpt, acoeff );
      efval = g2h_ExtFunctionalValued ( domain, 3, (double*)Bpt, bcoeff );
