...
int nfinalp;
point3d FinalCPoints[2*HOLE_K][(G2H_FINALDEG+1)*(G2H_FINALDEG+1)];

double iparams[3] = {0.0,0.0,0.45};
...
void DrawExtMatrix ( int k, int r, int s, double *Aii )
{
...
void OutPatch ( int n, int m, const point3d *cp, void *usrptr )
{
  ...
  memcpy ( FinalCPoints[nfinalp], cp,
          (G2H_FINALDEG+1)*(G2H_FINALDEG+1)*sizeof(point3d) );
  nfinalp ++;
} /*OutPatch*/
...
int main ()
{
  ...
  if ( (domain = gh_CreateDomaind ( HOLE_K, knots, Dompt )) ) {
    if ( g2h_ComputeBasisd ( domain ) ) {
      ...
      nfinalp = 0;
      if ( !g2h_NLFillHoled ( domain, Bpt, (double*)acoeff, NULL, OutPatch ) )
        goto failure;
      if ( !g2h_NLFunctionalValued ( domain, Bpt, acoeff, &funcval[1] ) )
        goto failure;      
      printf ( "E_c = %f, E_d = %f\n", funcval[1], funcval[2] );  
      if ( !g2h_NLExtFillHoled ( domain, Bpt, (double*)acoeff, NULL, OutPatch ) )
        goto failure;
      if ( !g2h_NLExtFunctionalValued ( domain, Bpt, acoeff, &funcval[1] ) )
        goto failure;
      printf ( "E_c = %f, E_d = %f\n", funcval[1], funcval[2] );
      times ( &stop );
      MakePictures ();
