...
int nfinalp;
point3f FinalCPoints[2*HOLE_K][(G2H_FINALDEG+1)*(G2H_FINALDEG+1)];

float iparams[3] = {0.0,0.0,0.45};
...
void DrawExtMatrix ( int k, int r, int s, float *Aii )
{
  ps_OpenFile ( "nlextmatrix.ps", 600 );
  DrawMatrix ( k, r, s, Aii );
  ps_CloseFile ();
  exit ( 0 );
} /*DrawExtMatrix*/
...
void OutPatch ( int n, int m, const point3f *cp, void *usrptr )
{
  ...
  memcpy ( FinalCPoints[nfinalp], cp,
          (G2H_FINALDEG+1)*(G2H_FINALDEG+1)*sizeof(point3f) );
  nfinalp ++;
} /*OutPatch*/
...
int i;
  ...
  if ( (domain = gh_CreateDomainf ( HOLE_K, knots, Dompt )) ) {
    if ( g2h_ComputeBasisf ( domain ) ) {
      ...
      nfinalp = 0;
      if ( !g2h_NLFillHolef ( domain, Bpt, (float*)acoeff, NULL, OutPatch ) )
        goto failure;
      if ( !g2h_NLFunctionalValuef ( domain, Bpt, acoeff, &funcval[1] ) )
        goto failure;
      printf ( "E_c = %f, E_d = %f\n", funcval[1], funcval[2] );
      if ( !g2h_NLExtFillHolef ( domain, Bpt, (float*)acoeff, NULL, OutPatch ) )
        goto failure;
      if ( !g2h_NLExtFunctionalValuef ( domain, Bpt, acoeff, &funcval[1] ) )
        goto failure;
      printf ( "E_c = %f, E_d = %f\n", funcval[1], funcval[2] );
      times ( &stop );
      MakePictures ();
...
if ( !g2h_NLExtFillHolef ( domain, Bpt, (float*)acoeff, NULL, OutPatch ) )
  goto failure;
if ( !g2h_NLExtFunctionalValuef ( domain, Bpt, acoeff, &funcval[1] ) )
  goto failure;printf ( "E_c = %f, E_d = %f\n", funcval[1], funcval[2] );
