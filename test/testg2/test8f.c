...
char fn[] = "g2hsplf.ps";
char datfn1[] = "g2hsplf.dat";
char datfn2[] = "g2hextf.dat";
char datfn3[] = "g2hf.dat";
...
int main ( void )
{
  ...
  if ( (domain = gh_CreateDomainf ( HOLE_K, knots, Dompt )) ) {
    if ( g2h_ComputeSplBasisf ( domain, NK, M1, M2 ) ) {
      nfbpatches = 0;
      if ( !g2h_FillHolef ( domain, 3, (float*)Bpt, NULL, NULL, OutputBezPatch ) )
        exit ( 1 );
      if ( !g2h_ExtFillHolef ( domain, 3, (float*)Bpt, NULL, NULL, OutputBezPatch ) )
        exit ( 1 );
      nfpatches = 0;
      if ( g2h_SplFillHolef ( domain, 3, (float*)Bpt, NULL, NULL, OutputSplPatch ) ) {
        times ( &stop );
