...
int main ()
{
  ...
  if ( (domain = gh_CreateDomaind ( HOLE_K, knots, Dompt )) ) {
    if ( g2h_ComputeBasisd ( domain ) ) {
      ...
      pkv_Selectd ( nfunc_a, 1, 5, 1, &bfder[1], &cmat[(2+3)*nfunc_a] );
      if ( !g2h_SetAltConstraintMatrixd ( domain, 3, 2, cmat ) )
        goto failure;
      cnstr[0] = -0.55;
      cnstr[1] = -0.0;
      ...
      memset ( cmat, 0, 6*(nfunc_a+nfunc_c)*sizeof(double) );
      pkv_Selectd ( nfunc_a, 1, 5, 1, bfder,
                    &cmat[2*(nfunc_c+nfunc_a)+nfunc_c] );
      pkv_Selectd ( nfunc_a, 1, 5, 1, &bfder[1],
                    &cmat[(2+3)*(nfunc_a+nfunc_c)+nfunc_c] );
      if ( !g2h_SetExtAltConstraintMatrixd ( domain, 3, 2, cmat ) )
        goto failure;
