...
int main ()
{
  ...
  if ( (domain = gh_CreateDomainf ( HOLE_K, knots, Dompt )) ) {
    if ( g2h_ComputeBasisf ( domain ) ) {
      ...
      pkv_Selectf ( nfunc_a, 1, 5, 1, bfder, &cmat[2*nfunc_a] );
      pkv_Selectf ( nfunc_a, 1, 5, 1, &bfder[1], &cmat[(2+3)*nfunc_a] );
      if ( !g2h_SetAltConstraintMatrixf ( domain, 3, 2, cmat ) )
        goto failure;
      cnstr[0] = -0.55;
      cnstr[1] = -0.0;
      if ( !g2h_FillHoleAltConstrf ( domain, 3, (float*)Bpt,
                                     2, cnstr, acoeff, NULL, OutPatch ) )
        goto failure;

      memset ( cmat, 0, 6*(nfunc_a+nfunc_c)*sizeof(float) );
      pkv_Selectf ( nfunc_a, 1, 5, 1, bfder,
                    &cmat[2*(nfunc_c+nfunc_a)+nfunc_c] );
      pkv_Selectf ( nfunc_a, 1, 5, 1, &bfder[1],
                    &cmat[(2+3)*(nfunc_a+nfunc_c)+nfunc_c] );
      if ( !g2h_SetExtAltConstraintMatrixf ( domain, 3, 2, cmat ) )
        goto failure;
      if ( !g2h_ExtFillHoleAltConstrf ( domain, 3, (float*)Bpt,
