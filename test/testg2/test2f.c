...
char fn1[] = "g2holeextf.ps";
char fn2[] = "g2extmatrixf.ps";
...
{
  ps_OpenFile ( fn2, 600 );
  g2h_DrawExtMatricesf ( domain, DrawMatrix );
  g2h_DrawExtMatricesf ( domain, ExtCondNumber );
  ps_CloseFile ();
  printf ( "%s\n", fn2 );
} /*MakePicture2*/
  ...
  if ( (domain = gh_CreateDomainf ( HOLE_K, knots, Dompt )) ) {
    if ( g2h_ComputeBasisf ( domain ) ) {
      if ( g2h_ComputeExtFormMatrixf ( domain ) ) {
        ...
        if ( !g2h_ExtFillHolef ( domain, 3, (float*)Bpt, acoeff, NULL, OutPatch ) )
          goto failure;
        times ( &stop );
        funcval = g2h_ExtFunctionalValuef ( domain, 3, (float*)Bpt, acoeff );
