  ...
  if ( (domain = gh_CreateDomaind ( HOLE_K, knots, Dompt )) ) {
    ...
    if ( g1h_ComputeBasisd ( domain ) ) {
      ...
      if ( !g1h_SetConstraintMatrixd ( domain, 2, cmat ) )
        goto failure;
      SetPoint3d ( &cnt[0], 0.25, 0.25, -0.8 );
      SetPoint3d ( &cnt[1], 1.0, 0.0, 1.0 );
      if ( !g1h_FillHoleConstrd ( domain, 3, (double*)Bpt,
      ...
      if ( !g1h_SetExtConstraintMatrixd ( domain, 2, cmat ) )
        goto failure;
      if ( !g1h_Q2ExtFillHoleConstrd ( domain, 3, (double*)Bpt,
