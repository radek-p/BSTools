...
void DrawAuxBasisFuncBSPatch ( int fn, float x, float y )
{
  ...
  ps_Write_Command ( s );
  ps_Set_Gray ( 0.5 );
  g1h_DrawSplBasAuxPatchesf ( domain, fn, DrawAuxBSPatch );
  ps_GRestore ();
...
void DrawBasisFuncBSPatch ( int fn, float x, float y )
{
  ...
  ps_Set_Gray ( 0.5 );
  g1h_DrawSplBasFunctionf ( domain, fn, DrawBSPatch );
  if ( fn >= nfunc_a+nfunc_b+nfunc_c ) {
    cnt = -((fn-(nfunc_a+nfunc_b+nfunc_c)) / (nfunc_d/HOLE_K));
    ps_Set_RGB ( 1.0, 0.0, 0.0 );
    g1h_DrawSplBasAuxPatchesf ( domain, fn, AltDrawAuxBSPatch );
  }
  ps_GRestore ();
...
void DrawBezPatchL1 ( int n, int m, const point3f *cp )
{
  float lap;

  lap = FindBezPatchMaxLaplacianf ( n, m, cp, 20, 20 );
  maxlap = max ( maxlap, lap );
} /*DrawBezPatchL1*/

void DrawBezPatchL2 ( int n, int m, const point3f *cp )
{
  void    *sp;
  point3f *p;
  int     i, s;
  ...
  for ( i = 0; i < s; i++ )
    p[i].z *= scf;
  DrawBezPatchLaplacianf ( n, m, p, 6, 6 );
  pkv_SetScratchMemTop ( sp );
} /*DrawBezPatchL2*/
...
void DrawBasisFuncBSPatchL ( int fn, float x, float y )
{
  ...
  if ( fn < nfunc_a || fn >= nfunc_a+nfunc_b ) {
    maxlap = 0.0;
    g1h_DrawSplBasFunctionf ( domain, fn, DrawBSPatchL1 );
    scf = 0.5/maxlap;
    g1h_DrawSplBasFunctionf ( domain, fn, DrawBSPatchL2 );
  }
  else {
    maxlap = 0.0;
    g1h_DrawBasBFunctionf ( domain, fn-nfunc_a, DrawBezPatchL1 );
    scf = 0.5/maxlap;
    g1h_DrawBasBFunctionf ( domain, fn-nfunc_a, DrawBezPatchL2 );
  }
  ps_GRestore ();
...
void DrawBPatchJ1 ( int n, int m, const point3f *cp )
{
  if ( nbfp < 64 ) {
    bfptab[nbfp].n = n;
    bfptab[nbfp].lkn = 0;
    memcpy ( bfptab[nbfp].cp, cp, (n+1)*(m+1)*sizeof(point3f) );
    nbfp ++;
  }
} /*DrawBPatchJ1*/

void DrawBSPatchJ1 ( int n, int lknu, const float *knu,
                     int m, int lknv, const float *knv, const point3f *cp )
{
  if ( nbfp < 64 ) {
    bfptab[nbfp].n = n;
    bfptab[nbfp].lkn = lknu;
    memcpy ( bfptab[nbfp].knots, knu, (lknu+1)*sizeof(float) );
    memcpy ( bfptab[nbfp].cp, cp, (lknu-n)*(lknv-m)*sizeof(point3f) );
    nbfp ++;
  }
} /*DrawBSPatchJ1*/
...
#define DD 32
  ...
  d = pkv_GetScratchMem ( (DD+1)*sizeof(point2f) );
  if ( !c || !d )
    exit ( 1 );
  for ( i = 0, j = nbfp-1;  i < nbfp;  j = i++ ) {
    for ( k = 0; k <= DD; k++ ) {
      u = (float)k/(float)DD;
      mbs_BCHornerPf ( bfptab[j].n, bfptab[j].n, 3, &bfptab[j].cp[0].x,
                       0.0, u, &p1.x );
      p1.z = ComputeBezLaplacianf ( bfptab[j].n, bfptab[j].n,
                       &bfptab[j].cp[0], 0.0, u );
      mbs_BCHornerPf ( bfptab[i].n, bfptab[i].n, 3, &bfptab[i].cp[0].x,
                       u, 0.0, &p2.x );
      p2.z = ComputeBezLaplacianf ( bfptab[i].n, bfptab[i].n,
                       &bfptab[i].cp[0], u, 0.0 );
      p1.z = scf*(p2.z-p1.z);
      CameraProjectPoint3f ( &CPos, &p1, &p2 );
      SetPoint2f ( &d[k], p2.x, p2.y );
      p1.z = 0.0;
      CameraProjectPoint3f ( &CPos, &p1, &p2 );
      SetPoint2f ( &c[k], p2.x, p2.y );
    }
    ps_Set_Gray ( 0.0 );
    ps_Draw_Polyline2f ( DD+1, c );
    ps_Set_RGB ( 0.0, 0.25, 1.0 );
    ps_Draw_Polyline2f ( DD+1, d );

    for ( k = 0; k <= DD; k++ ) {
      u = (float)k/(float)DD;
      mbs_BCHornerPf ( bfptab[i].n, bfptab[i].n, 3, &bfptab[i].cp[0].x,
                       u, 1.0, &p1.x );
      p1.z = ComputeBezLaplacianf ( bfptab[i].n, bfptab[i].n,
                       &bfptab[i].cp[0], u, 1.0 );
      p1.z *= -scf;
      CameraProjectPoint3f ( &CPos, &p1, &p2 );
      SetPoint2f ( &d[k], p2.x, p2.y );
...
#define DD 32
  ...
  for ( i = 0, j = HOLE_K-1;  i < HOLE_K;  j = i++ ) {
    for ( k = 0; k <= DD; k++ ) {
      u = (float)k/(float)DD;
      mbs_BCHornerPf ( bfptab[3*HOLE_K+j].n, bfptab[3*HOLE_K+j].n, 3,
                       &bfptab[3*HOLE_K+j].cp[0].x, 0.0, u, &p1.x );
      p1.z = ComputeBezLaplacianf ( bfptab[3*HOLE_K+j].n, bfptab[3*HOLE_K+j].n,
                       &bfptab[3*HOLE_K+j].cp[0], 0.0, u );
      mbs_BCHornerPf ( bfptab[3*HOLE_K+i].n, bfptab[3*HOLE_K+i].n, 3,
                       &bfptab[3*HOLE_K+i].cp[0].x, u, 0.0, &p2.x );
      p2.z = ComputeBezLaplacianf ( bfptab[3*HOLE_K+i].n, bfptab[3*HOLE_K+i].n,
    ...
    for ( k = 0; k <= DD; k++ ) {
      ...
      p1.z = ComputeBezLaplacianf ( bfptab[3*HOLE_K+i].n, bfptab[3*HOLE_K+i].n,
                       &bfptab[3*HOLE_K+i].cp[0], u, 1.0 );
      mbs_BCHornerPf ( bfptab[3*jj+1].n, bfptab[3*jj+1].n, 3,
                       &bfptab[3*jj+1].cp[0].x, 0.0, u, &p2.x );
      p2.z = ComputeBezLaplacianf ( bfptab[3*jj+1].n, bfptab[3*jj+1].n,
    ...
    for ( k = 0; k <= DD; k++ ) {
      ...
      mbs_BCHornerPf ( bfptab[3*i+2].n, bfptab[3*i+2].n, 3,
                       &bfptab[3*i+2].cp[0].x, 0.0, u, &p2.x );
      p2.z = ComputeBezLaplacianf ( bfptab[3*i+2].n, bfptab[3*i+2].n,
...
#define DD 32
  ...
  float   *b1, *b2, *c1, *c2, u, *knu, *knv;
  int     i, j, k, l, ll, m, n, lknu, lknv, start1, start2;
  point3f *cp1, *cp2, p1, p2;
  point2f *cc, *dd;
  ...
  ku = kv = mbs_NumKnotIntervalsf ( n, lknu, knu );
  pitch1 = (lknv-m)*3;
  pitch2 = (m+1)*3*kv;
  pitch3 = (m+1)*3;
  b1 = pkv_GetScratchMemf ( pitch2*ku*(n+1) );
  b2 = pkv_GetScratchMemf ( pitch2*ku*(n+1) );
  c1 = pkv_GetScratchMemf ( (n+1)*pitch3 );
  c2 = pkv_GetScratchMemf ( (n+1)*pitch3 );
  cc = pkv_GetScratchMem ( (DD+1)*sizeof(point2f) );
  dd = pkv_GetScratchMem ( (DD+1)*sizeof(point2f) );
  if ( b1 && b2 && c1 && c2 && cc && dd ) {
    for ( i = 0, j = HOLE_K-1;  i < HOLE_K;  j = i++ ) {
      cp1 = bfptab[j].cp;
      cp2 = bfptab[i].cp;
      mbs_BSPatchToBezf ( 3, n, lknu, knu, m, lknv, knv, pitch1, (float*)cp1,
                          &ku, NULL, NULL, &kv, NULL, NULL, pitch2, b1 );
      mbs_BSPatchToBezf ( 3, n, lknu, knu, m, lknv, knv, pitch1, (float*)cp2,
                          &ku, NULL, NULL, &kv, NULL, NULL, pitch2, b2 );
      for ( k = 0; k < ku; k++ ) {
        start1 = k*pitch3;
        pkv_Selectf ( n+1, pitch3, pitch2, pitch3, &b1[start1], c1 );
        start2 = k*(n+1)*pitch2;
        pkv_Selectf ( n+1, pitch3, pitch2, pitch3, &b2[start2], c2 );
        for ( ll = 0; ll <= DD; ll++ ) {
          u = (float)ll/(float)DD;
          mbs_BCHornerPf ( n, n, 3, c1, 0.0, u, &p1.x );
          p1.z = ComputeBezLaplacianf ( n, n, (point3f*)c1, 0.0, u );
          mbs_BCHornerPf ( n, n, 3, c2, u, 0.0, &p2.x );
          p2.z = ComputeBezLaplacianf ( n, n, (point3f*)c2, u, 0.0 );
          p1.z = scf*(p2.z-p1.z);
          CameraProjectPoint3f ( &CPos, &p1, &p2 );
          SetPoint2f ( &dd[ll], p2.x, p2.y );
          p1.z = 0.0;
          CameraProjectPoint3f ( &CPos, &p1, &p2 );
          SetPoint2f ( &cc[ll], p2.x, p2.y );
        }
        ps_Set_Gray ( 0.0 );
        ps_Draw_Polyline2f ( DD+1, cc );
        ps_Set_RGB ( 0.0, 0.25, 1.0 );
        ps_Draw_Polyline2f ( DD+1, dd );
      }

      for ( k = 1; k < ku; k++ ) {
        for ( l = 0; l < kv; l++ ) {
          start1 = (k-1)*(n+1)*pitch2+l*pitch3;
          pkv_Selectf ( n+1, pitch3, pitch2, pitch3, &b2[start1], c1 );
          start2 = k*(n+1)*pitch2+l*pitch3;
          pkv_Selectf ( n+1, pitch3, pitch2, pitch3, &b2[start2], c2 );
          for ( ll = 0; ll <= DD; ll++ ) {
            u = (float)ll/(float)DD;
            mbs_BCHornerPf ( n, n, 3, c1, 1.0, u, &p1.x );
            p1.z = ComputeBezLaplacianf ( n, n, (point3f*)c1, 1.0, u );
            mbs_BCHornerPf ( n, n, 3, c2, 0.0, u, &p2.x );
            p2.z = ComputeBezLaplacianf ( n, n, (point3f*)c2, 0.0, u );
            p1.z = scf*(p2.z-p1.z);
            CameraProjectPoint3f ( &CPos, &p1, &p2 );
            SetPoint2f ( &dd[ll], p2.x, p2.y );
            p1.z = 0.0;
      ...
      for ( k = 0; k < ku; k++ )
        for ( l = 1; l < kv; l++ ) {
          ...
          for ( ll = 0; ll <= DD; ll++ ) {
            u = (float)ll/(float)DD;
            mbs_BCHornerPf ( n, n, 3, c1, u, 1.0, &p1.x );
            p1.z = ComputeBezLaplacianf ( n, n, (point3f*)c1, u, 1.0 );
            mbs_BCHornerPf ( n, n, 3, c2, u, 0.0, &p2.x );
            p2.z = ComputeBezLaplacianf ( n, n, (point3f*)c2, u, 0.0 );
            p1.z = scf*(p2.z-p1.z);
            CameraProjectPoint3f ( &CPos, &p1, &p2 );
...
void DrawBasisFuncBSPatchLJ ( int fn, float x, float y )
{
  char s[50];
  ...
  nbfp = 0;
  if ( fn < nfunc_a ) {
    g1h_DrawBasAFunctionf ( domain, fn, DrawBPatchJ1 );
    DrawBasAJump ();
  }
  else if ( fn < nfunc_a+nfunc_b ) {
    g1h_DrawBasBFunctionf ( domain, fn-nfunc_a, DrawBPatchJ1 );
    DrawBasBJump ();
  }
  else {
    g1h_DrawSplBasFunctionf ( domain, fn, DrawBSPatchJ1 );
    DrawBasCDJump ();
  }
  ps_GRestore ();
...
{
  ps_OpenFile ( fn3, 600 );
  g1h_DrawSplMatricesf ( domain, DrawSplMatrix );
  ps_CloseFile ();
  printf ( "%s\n", fn3 );
} /*MakePicture4*/
...
void AllocPatchStorage ( void )
{
  ...
  for ( i = 0; i < HOLE_K; i++ ) {
    knu1[i] = malloc ( (n+G1H_FINALDEG+1)*sizeof(float) );
    knv1[i] = malloc ( (n+G1H_FINALDEG+1)*sizeof(float) );
    scp1[i] = malloc ( n*n*sizeof(point3f) );
    knu2[i] = malloc ( (n+G1H_FINALDEG+1)*sizeof(float) );
    knv2[i] = malloc ( (n+G1H_FINALDEG+1)*sizeof(float) );
...
float CompIntegral ( void )
{
  ...
  float    lap1, lap2;
  float    jac1, jac2, gx, gy, gxx, gxy, gyy;
  FILE     *f;
  ...
  ap1 = pkv_GetScratchMem ( 3*sz*sizeof(point3f) );
  ap2 = pkv_GetScratchMem ( 3*sz*sizeof(point3f) );
  if ( !ap1 || !ap2 )
    exit ( 1 );
  sss = 0.0;
  for ( i = 0; i < HOLE_K; i++ ) {
    if ( IsZnonZero ( sdegu1[i], lknu1[i], knu1[i],
      ...
      for ( j = 0; j < QUAD_FACTOR*(NK+1); j++ ) {
        ...
        mbs_multideBoorDer2f ( sdegu1[i], lknu1[i], knu1[i], 1, 3*d1, 0,
            (float*)scp1[i], u, &ap1[0].x, &ap1[d1].x, &ap1[2*d1].x );
        mbs_multideBoorDer2f ( sdegu2[i], lknu2[i], knu2[i], 1, 3*d2, 0,
            (float*)scp2[i], u, &ap2[0].x, &ap2[d2].x, &ap2[2*d2].x );
        s = 0.0;
        for ( k = 0; k < QUAD_FACTOR*(NK+1); k++ ) {
          v = (float)(k+k+1)/(float)(2*QUAD_FACTOR*(NK+1));
          mbs_multideBoorDer2f ( sdegv1[i], lknv1[i], knv1[i], 3, 3, 3*d1,
              (float*)ap1, v, &p1[0].x, &p1[4].x, &p1[8].x );
          mbs_multideBoorDer2f ( sdegv2[i], lknv2[i], knv2[i], 3, 3, 3*d2,
              (float*)ap2, v, &p2[0].x, &p2[4].x, &p2[8].x );
          pkn_Comp2iDerivatives2f ( p1[1].x, p1[1].y, p1[4].x, p1[4].y,
              p1[2].x, p1[2].y, p1[5].x, p1[5].y, p1[8].x, p1[8].y,
              1, &p1[1].z, &p1[4].z, &p1[2].z, &p1[5].z, &p1[8].z,
              &gx, &gy, &gxx, &gxy, &gyy );
          lap1 = gxx+gyy;
          jac1 = fabs(p1[4].x*p1[1].y-p1[4].y*p1[1].x);
          pkn_Comp2iDerivatives2f ( p2[1].x, p2[1].y, p2[4].x, p2[4].y,
...
/* a better chance of being programmed without bugs */
  ...
  for ( i = 0; i < nfunc_a+nfunc_c+nfunc_d; i++ ) {
    ...
    cnt = 0;
    g1h_DrawSplBasFunctionf ( domain, fni, SavePatch1 );
    for ( j = 0; j <= i; j++ ) {
...
dumpspl = (boolean)(i == 0 && j == 0);
      ...
      if ( k >= 0 ) {  /* this element belongs to a nonzero block of A */
        cnt = 0;
        g1h_DrawSplBasFunctionf ( domain, fnj, SavePatch2 );
...
dumpspl = false;
    for ( j = 0; j < nfunc_b; j++ ) {
      cnt = 0;
      g1h_DrawSplBasFunctionf ( domain, nfunc_a+j, SavePatch2 );
...
{
  AllocPatchStorage ();
  g1h_DrawSplMatricesf ( domain, SaveMatrices );
  WriteBlockAddr ();
