...
char fn[] = "eg1hspld.ps";
char datfn1[] = "eg1hspld.dat";
char datfn2[] = "eg1hextd.dat";
char datfn3[] = "eg1hd.dat";
...
int main ( void )
{
  ...
  if ( (domain = gh_CreateDomaind ( HOLE_K, knots, Dompt )) ) {
    if ( g1h_ComputeSplBasisd ( domain, NK, M1, M2 ) ) {
      d0 = g1h_V0SpaceDimd ( domain );
      d1 = g1h_ExtV0SpaceDimd ( domain );
      printf ( "d0=%d, d1=%d\n", d0, d1 );
      nfbpatches = 0;
      if ( !g1h_FillHoled ( domain, 3, (double*)Bpt, acoeff, NULL, OutputBezPatch ) )
        exit ( 1 );
      fval = g1h_FunctionalValued ( domain, 3, (double*)Bpt, acoeff );
      printf ( "fval = %f, ", fval );

      memmove ( &acoeff[3*(d1-d0)], acoeff, 3*d0*sizeof(double) );
      memset ( acoeff, 0, 3*(d1-d0)*sizeof(double) );
      fval = g1h_ExtFunctionalValued ( domain, 3, (double*)Bpt, acoeff );
      printf ( "%f\n", fval );

      if ( !g1h_ExtFillHoled ( domain, 3, (double*)Bpt, acoeff, NULL, OutputBezPatch ) )
        exit ( 1 );
      fval = g1h_ExtFunctionalValued ( domain, 3, (double*)Bpt, acoeff );
      printf ( "fval = %f\n", fval );
      nfpatches = 0;
      if ( g1h_SplFillHoled ( domain, 3, (double*)Bpt, NULL, NULL, OutputSplPatch ) ) {
        times ( &stop );
