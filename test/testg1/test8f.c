...
char fn[] = "eg1hsplf.ps";
char datfn1[] = "eg1hsplf.dat";
char datfn2[] = "eg1hextf.dat";
char datfn3[] = "eg1hf.dat";
...
int main ( void )
{
  ...
  if ( (domain = gh_CreateDomainf ( HOLE_K, knots, Dompt )) ) {
    if ( g1h_ComputeSplBasisf ( domain, NK, M1, M2 ) ) {
      nfbpatches = 0;
      if ( !g1h_FillHolef ( domain, 3, (float*)Bpt, acoeff, NULL, OutputBezPatch ) )
        exit ( 1 );
      fval = g1h_FunctionalValuef ( domain, 3, (float*)Bpt, acoeff );
      printf ( "fval = %f\n", fval );
      if ( !g1h_ExtFillHolef ( domain, 3, (float*)Bpt, acoeff, NULL, OutputBezPatch ) )
        exit ( 1 );
      fval = g1h_ExtFunctionalValuef ( domain, 3, (float*)Bpt, acoeff );
      printf ( "fval = %f\n", fval );
      nfpatches = 0;
      if ( g1h_SplFillHolef ( domain, 3, (float*)Bpt, NULL, NULL, OutputSplPatch ) ) {
        times ( &stop );
