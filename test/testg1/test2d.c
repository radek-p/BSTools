...
char fn1[] = "g1holeextd.ps";
char fn2[] = "g1extmatrixd.ps";
...
{
  ps_OpenFile ( fn2, 600 );
  g1h_DrawExtMatricesd ( domain, DrawMatrix );
  g1h_DrawExtMatricesd ( domain, ExtCondNumber );
  ps_CloseFile ();
  printf ( "%s\n", fn2 );
} /*MakePicture2*/
  ...
  if ( (domain = gh_CreateDomaind ( HOLE_K, knots, Dompt )) ) {
    if ( g1h_ComputeBasisd ( domain ) ) {
      if ( g1h_ComputeExtFormMatrixd ( domain ) ) {
        ...
        if ( !g1h_ExtFillHoled ( domain, 3, (double*)Bpt, acoeff, NULL, OutPatch ) )
          goto failure;
        times ( &stop );
        funcval = g1h_ExtFunctionalValued ( domain, 3, (double*)Bpt, acoeff );       
