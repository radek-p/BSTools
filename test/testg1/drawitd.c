...
/* //////////////////////////g//////////////////////////////////////////////// */
double FindBezPatchMaxLaplaciand ( int n, int m, const point3d *cp, int d0, int d1 )
{
  int     k, l;
  double   lap, maxval;
  ...
  for ( k = 0; k <= d0; k++ )
    for ( l = 0; l <= d1; l++ ) {
      lap = ComputeBezLaplaciand ( n, m, cp,
                      (double)k/(double)d0, (double)l/(double)d1 );
      maxval = max ( maxval, fabs(lap) );
    }
  return maxval;
} /*FindBezPatchMaxLaplaciand*/

void DrawBezPatchLaplaciand ( int n, int m, const point3d *cp, int d0, int d1 )
{
#define DD 32
  void    *sp;
  double   u, v;
  ...
  if ( cc ) {
    for ( k = 0; k <= d0; k++ ) {
      ...
      for ( l = 0; l <= DD; l++ ) {
        ...
        p.z = ComputeBezLaplaciand ( n, m, cp, u, v );
        CameraProjectPoint3d ( &CPos, &p, &q );
        SetPoint2d ( &cc[l], q.x, q.y );
      }
      SetCLW ( k, 0, d0 );
