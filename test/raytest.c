...
static void SetCPos ( void )
{
  ...
  CameraRotZCf ( &CPos, -3*PI/4 );
  CameraRotXCf ( &CPos, PI );
  SetVector3f ( &v, 0.4, 0.0, 0.0 );
  CameraMoveCf ( &CPos, &v );
...
/*
static void DrawBezNet ( point4f *p )
{
  int    i;
  point3f q;
  point2f r[36];
...
static void DrawRBezTree ( RBezPatchTreef *tree, float w, float h, float x, float y )
{
  ps_Set_Gray ( 0.0 );
...
{
  RBezPatchTreeVertexfp a, b, c, d, e, f, g, h, i;

  a = tree->root;
  b = rbez_GetRBezLeftVertexf ( tree, a );
  c = rbez_GetRBezRightVertexf ( tree, a );

  d = rbez_GetRBezLeftVertexf ( tree, b );
  e = rbez_GetRBezRightVertexf ( tree, b );
  f = rbez_GetRBezLeftVertexf ( tree, c );
  g = rbez_GetRBezRightVertexf ( tree, c );

  h = rbez_GetRBezLeftVertexf ( tree, d );
  i = rbez_GetRBezRightVertexf ( tree, d );
