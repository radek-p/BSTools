  ...
  for ( i = m+1; i < MM-m; i++ ) {
    if ( v[i] > v[i-1] ) {
      ...
      ps_Draw_Line ( (float)aux[0].x, (float)aux[0].y, (float)aux[1].x, (float)aux[1].y );
                             ...
                             /* draw knots */
  psl_SetLine ( (float)frame[0].x, (float)fr[0].y, (float)(frame[0].x+fv[0].x),
                (float)fr[0].y, 0.0, 1.0 );
  for ( i = 1, k = 1; i <= NN; i++ ) {
    if ( u[i] > u[i-1] ) {
      if ( i < NN )
        psl_Tick ( (float)u[i] );
...
int main ()
{
  ...
  NNa = mbs_LastknotMaxInsd ( n, NN, u, &ku );
  MMa = mbs_LastknotMaxInsd ( m, MM, v, &kv );
  pitch = 3*kv*(m+1);
  ua = (double*)pkv_GetScratchMem ( (NNa+1)*sizeof(double) );
  va = (double*)pkv_GetScratchMem ( (MMa+1)*sizeof(double) );
  cpa = pkv_GetScratchMemd ( 3*ku*kv*(n+1)*(m+1) );
  mbs_BSPatchToBezd ( 3, n, NN, u, m, MM, v, 3*(MM-m), (double*)&cp[0][0],
  ...
  DisplayBSPatch ( n, NN, u, m, MM, v, (double*)cp, 6, 6, 4, 6.0, 2.0 );
  ps_Set_Gray ( 0.0 );
  DisplayBSCNet ( n, NNa, m, MMa, (double*)cpa, 4.0 );
