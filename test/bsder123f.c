...
static void DrawBSCurve ( int degree, int lastknot, const float *u, const point2f *d )
{
#define DD 200
  int     i;
  float   t;
...
/*
static void Test1 ( float gr, int n, float *d )
{
  int i;
  ...
  if ( n )
    ps_Draw_Polyline2f ( n+1, (point2f*)d );
  for ( i = 0; i <= n; i++ )
...
/*  exit ( 0 );*/
  ...
  for ( i = 0; i <= DD; i++ ) {
    ...
    mbs_deBoorDerC2f ( n, N, u, d, t, &p, &d1 );
  ...
  for ( i = 0; i <= DD; i++ ) {
    t = u[n] + (float)i/(float)DD*(u[N-n]-u[n]);
    mbs_deBoorDer2C2f ( n, N, u, (float*)d, t, &p, &d1, &d2 );
    ps_Fill_Circle ( p.x, p.y, 8.0 );
    psl_SetLine ( p.x, p.y, p.x+d1.x, p.y+d1.y, 0.0, 1.0 );
    psl_Draw ( 0.0, 0.97, 4.0 );
    psl_Arrow ( 1.0, true );
