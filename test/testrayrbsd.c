...
point4d cpoints[] =
  {{-3.0,-3.0,-0.5,1.0}, {-2.7,-3.0,-0.5,1.0}, {-1.5,-3.0,-0.5,1.0}, {0.0,-3.0,-0.5,1.0},
   {1.5,-3.0,-0.5,1.0}, {2.7,-3.0,-0.5,1.0}, {3.0,-3.0,-0.5,1.0},
   {-3.0,-2.7,0.0,1.0}, {-2.7,-2.7,0.0,1.0}, {-1.5,-2.7,0.0,1.0}, {0.0,-2.7,0.0,1.0}, {1.5,-2.7,0.0,1.0},
   {2.7,-2.7,0.0,1.0}, {3.0,-2.7,0.0,1.0},
   {-3.0,-1.5,0.1,1.0}, {-2.7,-1.5,0.1,1.0}, {-1.5,-1.5,0.1,1.0}, {0.0,-1.5,0.1,1.0},
   {3.0,-3.0,5.0,2.0}, {1.35,-0.75,0.05,0.5}, {3.0,-1.5,0.1,1.0},
   {-3.0,0.0,0.2,1.0}, {-2.7,0.0,0.2,1.0}, {-1.5,0.0,0.2,1.0}, {0.0,0.0,0.2,1.0},
   {1.5,0.0,0.2,1.0}, {2.7,0.0,0.2,1.0}, {3.0,0.0,0.2,1.0},
   {-3.0,1.5,0.1,1.0}, {-2.7,1.5,0.1,1.0}, {-1.5,1.5,0.1,1.0}, {0.0,1.5,0.1,1.0},
   {1.5,1.5,0.1,1.0}, {2.7,1.5,0.1,1.0}, {3.0,1.5,0.1,1.0},
   {-3.0,2.7,0.0,1.0}, {-2.7,2.7,0.0,1.0}, {-1.5,2.7,0.0,1.0}, {0.0,2.7,0.0,1.0}, {1.5,2.7,0.0,1.0}, {2.7,2.7,0.0,1.0},
   {3.0,2.7,0.0,1.0},
   {-3.0,3.0,-0.5,1.0}, {-2.7,3.0,-0.5,1.0}, {-1.5,3.0,-0.5,1.0}, {0.0,3.0,-0.5,1.0},
   {1.5,3.0,-0.5,1.0}, {2.7,3.0,-0.5,1.0}, {3.0,3.0,-0.5,1.0}};
...
void DrawRBezTreeVertexDivision ( RBezPatchTreeVertexd *vertex,
                                  double w, double h, double x, double y )
{
  ...
  if ( vertex->left ) {
    ...
    if ( vertex->divdir == 0 ) {
      ...
      DrawRBezTreeVertexDivision ( vertex->left, w1, h, x, y );
      DrawRBezTreeVertexDivision ( vertex->right, w-w1, h, x+w1, y );
    }
    else {
      h1 = h*(vertex->left->v1-vertex->left->v0)/(vertex->v1-vertex->v0);
      ps_Draw_Line ( (float)x, (float)(y+h1), (float)(x+w), (float)(y+h1) );
      DrawRBezTreeVertexDivision ( vertex->left, w, h1, x, y );
      DrawRBezTreeVertexDivision ( vertex->right, w, h-h1, x, y+h1 );
    }
  }
} /*DrawRBezTreeVertexDivision*/

void DrawRBezTree ( RBezPatchTreed *tree, double w, double x, double y )
{
  double h;
  ...
  DrawRBezTreeVertexDivision ( tree->root, w, h, x, y );
} /*DrawRBezTree*/
...
int main ( void )
{
  ...
  times ( &start );
  tree = rbez_NewRBSPatchTreed ( 0, n, lknu, knots_u, m, lknv, knots_v,
                                 lknv-m, cpoints );
  RayTrace ();
  DrawRBezTree ( tree, 480, 650, 0 );
