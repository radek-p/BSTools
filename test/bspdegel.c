...
int main ()
{
  ...
  ku = mbs_NumKnotIntervalsf ( n, NN, u );
  kv = mbs_NumKnotIntervalsf ( m, MM, v );
  ua = pkv_GetScratchMem ( (NN+2+ku)*sizeof(float) );
  va = pkv_GetScratchMem ( (MM+2+kv)*sizeof(float) );
  cpa = pkv_GetScratchMem ( (NN-n+ku)*(MM-m+kv)*sizeof(point3f) );
  ...
  mbs_multiBSDegElevf ( 1, 3*(MM-m), n, NN, u, 0, (float*)cp, 1,
                        &na, &NNa, ua, 0, cpa, false );
  ps_Set_Gray ( 0.68 );
  DisplayBSPatch ( na, NNa, ua, m, MM, v, (float*)cpa, 6, 6, 4, 6.0, 2.0 );
  ps_Set_Gray ( 0.0 );
  DisplayBSCNet ( na, NNa, m, MM, (float*)cpa, 4.0 );
  DisplayBSDomain ( na, NNa, ua, m, MM, v, frame );
  ps_GRestore ();

  ps_GSave ();
  ps_Write_Command ( "0 500 translate" );
  mbs_multiBSDegElevf ( NN-n, 3, m, MM, v, 3*(MM-m), (float*)cp, 1,
                        &ma, &MMa, va, 3*(MM-m+kv), cpa, false );
  ps_Set_Gray ( 0.68 );
  DisplayBSPatch ( n, NN, u, ma, MMa, va, (float*)cpa, 6, 6, 4, 6.0, 2.0 );
  ps_Set_Gray ( 0.0 );
  DisplayBSCNet ( n, NN, ma, MMa, (float*)cpa, 4.0 );
  ...
  mbs_multiBSDegElevf ( NN-n, 3, m, MM, v, 3*(MM-m), (float*)cp, 1,
                        &ma, &MMa, va, 3*(MM-m+kv), cpa, false );
  mbs_multiBSDegElevf ( 1, 3*(MMa-ma), n, NN, u, 0, NULL, 1,
                        &na, &NNa, ua, 0, cpa, false );
  ps_Set_Gray ( 0.68 );
  DisplayBSPatch ( na, NNa, ua, ma, MMa, va, (float*)cpa, 6, 6, 4, 6.0, 2.0 );
  ps_Set_Gray ( 0.0 );
  DisplayBSCNet ( na, NNa, ma, MMa, (float*)cpa, 4.0 );
