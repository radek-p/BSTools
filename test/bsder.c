...
float kn[NN+1] = {-0.5,0.0,0.0,0.0,2.4,2.4,2.4,2.4,2.4,6.0,6.0,6.0,9.0};
...
static void DrawArc ( int nn, int NNN, float *kn, point2f *cp,
                      float t0, float t1 )
{
#define DD 50
  int i;
  ...
  for ( i = 0; i <= DD; i++ ) {
    ...
    mbs_deBoorC2f ( nn, NNN, kn, cp, t, &c[i] );
  }
...
static void ShowIt ( int dy )
{
  ...
  mbs_FindBSDerivativeC2f ( n, NNN, kn, cp, NULL, dkn, dcp );
  ...
  ps_Set_Line_Width ( 3.0 );
  ps_Set_Gray ( 0.0 );
  ps_Draw_Polyline2f ( NNN-n-1, dcp );
  ps_Set_Gray ( 0.0 );
  for ( i = 0; i < NNN-n-1; i++ )
    ps_Mark_Circle ( dcp[i].x, dcp[i].y );
