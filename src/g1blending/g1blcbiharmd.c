...
failure:
  ...
  *arow = NULL;
  return false;
} /*g1bl_SetupClosedBiharmAMatrixd*/
                               ...
                               double *rhs )
{
  ...
  for ( i = 0; i < blnum; i++ ) {
    for ( j = 0; j < 5; j++ ) {
      ...
      pkn_MultMatrixSubd ( 1, 2, 0, &coeff[5*k], spdimen, spdimen,
                 &cpoints[i*pitch], 0, &rhs[l*bldim*spdimen] );
      if ( bldim > 1 )
        pkn_MultMatrixSubd ( 1, 1, 0, &coeff[5*k], spdimen, spdimen,
                 &cpoints[i*pitch+spdimen], 0, &rhs[(l*bldim+1)*spdimen] );
      pkn_MultMatrixSubd ( 1, 2, 0, &coeff[5*k+3], spdimen, spdimen,
                 &cpoints[i*pitch+(lastknotv-4)*spdimen], 0,
                 &rhs[((l+1)*bldim-1)*spdimen]);
      if ( bldim > 1 )
        pkn_MultMatrixSubd ( 1, 1, 0, &coeff[5*k+4], spdimen, spdimen,
                 &cpoints[i*pitch+(lastknotv-4)*spdimen], 0,
                 &rhs[((l+1)*bldim-2)*spdimen] );
    }
  }
  return true;
} /*g1bl_SetupClosedBiharmRHSd*/
