                            ...
                            double *rhs )
{
  ...
  if ( bldim > 1 ) {
    ...
    pkn_MultMatrixSubd ( 1, 1, 0, &coeff[c1+4], spdimen, spdimen,
                 &cpoints[(i+2)*pitch+(bldim+2)*spdimen], 0,
                 &rhs[((j+1)*bldim-2)*spdimen] );
  }
  pkn_MultMatrixSubd ( 1, 2, 0, &coeff[c1+3], spdimen, spdimen,
               &cpoints[(i+2)*pitch+(bldim+2)*spdimen], 0,
                               ...
                               double *rhs )
{
  ...
  for ( j = 0; j < bldim; j++ ) {
    pkn_MultMatrixSubd ( 1, 5, 0, &coeff[0], spdimen, spdimen,
                 &cpoints[j*spdimen], 0, &rhs[j*spdimen] );
    pkn_MultMatrixSubd ( 1, 5, 0, &coeff[5], spdimen, spdimen,
