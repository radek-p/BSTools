...
/* ///////////////////////////////////////////////////////////////////////// */
/* The procedure pkn_Block1CholeskyDecompMf computes the Cholesky            */
                                ...
                                int spdimen, int xpitch, float *x )
{
  ...
  for ( b = 0; b < k; b++ ) {
    ...
    for ( j = 0; j < r; j++ ) {      /* this concerns the column b*r+j */
      lj0 = lbb + pkn_SymMatIndex ( j, 0 );
      pkn_MultMatrixNumf ( 1, spdimen, 0, &x[(b*r+j)*xpitch],
                       1.0/A[lj0+j], 0, &x[(b*r+j)*xpitch] );
      for ( i = j+1; i < r; i++ ) {
        li0 = lbb + pkn_SymMatIndex ( i, 0 );
        pkn_AddMatrixMf ( 1, spdimen, 0, &x[(b*r+i)*xpitch],
                          0, &x[(b*r+j)*xpitch], -A[li0+j],
                          0, &x[(b*r+i)*xpitch] );
      }
      for ( i = 0; i < s; i++ ) {
        li0 = lkb + r*i;
        pkn_AddMatrixMf ( 1, spdimen, 0, &x[(k*r+i)*xpitch],
  ...
  for ( j = 0; j < s; j++ ) {
    lj0 = pkn_SymMatIndex ( j, 0 );
    pkn_MultMatrixNumf ( 1, spdimen, 0, &x[(k*r+j)*xpitch], 1.0/Akk[lj0+j],
                         0, &x[(k*r+j)*xpitch] );
    for ( i = j+1; i < s; i++ ) {
      li0 = pkn_SymMatIndex ( i, 0 );
      pkn_AddMatrixMf ( 1, spdimen, 0, &x[(k*r+i)*xpitch],
                                ...
                                int spdimen, int xpitch, float *x )
{
  ...
  for ( j = s-1; j >= 0; j-- ) {
    lj0 = pkn_SymMatIndex ( j, 0 );
    pkn_MultMatrixNumf ( 1, spdimen, 0, &x[(k*r+j)*xpitch],
  ...
  for ( b = k-1; b >= 0; b-- ) {
    ...
    for ( j = r-1; j >= 0; j-- ) {
      lj0 = lbb + pkn_SymMatIndex ( j, 0 );
      pkn_MultMatrixNumf ( 1, spdimen, 0, &x[(b*r+j)*xpitch], 1.0/A[lj0+j],
                           0, &x[(b*r+j)*xpitch] );
      for ( i = j-1; i >= 0; i-- ) {
        pkn_AddMatrixMf ( 1, spdimen, 0, &x[(b*r+i)*xpitch],
                                   ...
                                   int ypitch, float *y )
{
  ...
  t = r*(r+1)/2;
  for ( i = 0; i < k; i++ )
    pkn_SymMatrixMultf ( r, &A[i*t], spdimen,
                         xpitch, &x[i*r*xpitch], ypitch, &y[i*r*ypitch] );
  pkn_SymMatrixMultf ( s, Akk, spdimen,
                       xpitch, &x[k*r*xpitch], ypitch, &y[k*r*ypitch] );
  for ( i = 0; i < k; i++ ) {
    pkn_MultMatrixf ( s, r, r, &Aki[i*r*s], spdimen,
                      xpitch, &x[i*r*xpitch], spdimen, z );
    pkn_AddMatrixf ( s, spdimen, spdimen, z,
                     ypitch, &y[k*r*ypitch], ypitch, &y[k*r*ypitch] );
    pkn_MultTMatrixf ( s, r, r, &Aki[i*r*s], spdimen,
                       xpitch, &x[k*r*xpitch], spdimen, z );
    pkn_AddMatrixf ( r, spdimen, spdimen, z,
