                                ...
                                float **QA21 )
{
  ...
  float *v, *p, gamma, s;
  float **rows11a, **rows11b, **rows22a, **rows22b, *m21a, *m21b;
  void  *aa, *ab;
  ...
  rows11a = pkv_GetScratchMem ( n*sizeof(float*) );
  rows11b = pkv_GetScratchMem ( n*sizeof(float*) );
  ...
  size21 = w*(n-w);
  PKV_MALLOC ( aa, (size11a+size22a+size21)*sizeof(float) );
  if ( !aa )
    goto failure;
  rows11a[0] = aa;
  rows22a[0] = &rows11a[0][size11a];
  m21a = &rows22a[0][size22a];
  pkn_NRBFindRowsf ( w, prof11a, rows11a[0], rows11a );
  pkn_NRBFindRowsf ( n-w, prof22a, rows22a[0], rows22a );

        /* copy the matrix A, splitting it to blocks */
  memcpy ( rows11a[0], Amat, size11a*sizeof(float) );
  memset ( m21a, 0, size21*sizeof(float) );
  for ( j = w; j < n; j++ ) {
    k = prof[j];
    if ( k < w ) {
      memcpy ( &m21a[(j-w)*w+k], &Arows[j][k], (w-k)*sizeof(float) );
      k = prof11a[j];
      memcpy ( &rows22a[j-w][k], &Arows[j][w+k], (j-w-k+1)*sizeof(float) );
    }
    else
      memcpy ( &rows22a[j-w][k-w], &Arows[j][k], (j-k+1)*sizeof(float) );
  }
  ...
  for ( i = 0; i < w; i++ ) {
    ...
    pkn_NRBSymMultf ( w, prof11a, rows11a[0], rows11a, 1, 1, v, 1, p );
    pkn_MultTMatrixAddf ( n-w, w, w, m21a, 1, 1, &v[w], 1, p );
    pkn_NRBSymMultf ( n-w, prof22a, rows22a[0], rows22a, 1, 1, &v[w], 1, &p[w] );
    pkn_MultMatrixAddf ( n-w, w, w, m21a, 1, 1, v, 1, &p[w] );
    ...
    PKV_MALLOC ( ab, (size11b+size22b+size21)*sizeof(float) );
    if ( !ab )
      goto failure;
    rows11b[0] = ab;
    rows22b[0] = &rows11b[0][size11b];
    m21b = &rows22b[0][size22b];
    pkn_NRBFindRowsf ( w, prof11b, rows11b[0], rows11b );
    pkn_NRBFindRowsf ( n-w, prof22b, rows22b[0], rows22b );
          /* copy the matrix A_{k-1} */
    memset ( rows11b[0], 0, (size11b+size22b)*sizeof(float) );
    for ( j = 0; j < w; j++ )
      memcpy ( &rows11b[j][prof11a[j]], &rows11a[j][prof11a[j]],
               (j-prof11a[j]+1)*sizeof(float) );
    for ( j = 0; j < n-w; j++ )
      memcpy ( &rows22b[j][prof22a[j]], &rows22a[j][prof22a[j]],
               (j-prof22a[j]+1)*sizeof(float) );
    memcpy ( m21b, m21a, size21*sizeof(float) );
    ...
    memcpy ( rows11a, rows11b, n*sizeof(float*) );
    m21a = m21b;
    aa = rows11a[0];
    ab = NULL;
  }
  ...
  memcpy ( QA11rows, rows11a, w*sizeof(float*) );
  memcpy ( qa22prof, prof22a, (n-w)*sizeof(int) );
  memcpy ( QA22rows, rows22a, (n-w)*sizeof(float*) );
  *QA21 = m21a;
