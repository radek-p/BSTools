...
#define NU 1.0e-7
  ...
  float   *cd, *sd, *dd1, *dd2;
  int     i, j, k, p, q;
  float   c, s, mu1, mu, sigma;
  boolean newblock;

  sp = pkv_GetScratchMemTop ();
  if ( n < 2 )
    goto failure;
  cd = pkv_GetScratchMemf ( 4*n-5 );
  if ( !cd )
    goto failure;
  sd = &cd[n-1];
  dd1 = &sd[n-1];
  ...
  for ( j = 0; j < n-1; j++ )
    for ( i = j+2; i < n; i++ ) {
      ...
      pkn_FindGivensRotationf ( a[pkn_LowerTrMatIndex(j+1,j)],
                                a[pkn_LowerTrMatIndex(i,j)], &c, &s );
          /* apply the rotation */
      a[pkn_LowerTrMatIndex(j+1,j)] =
          c*a[pkn_LowerTrMatIndex(j+1,j)] - s*a[pkn_LowerTrMatIndex(i,j)];
      for ( k = j+2; k < i; k++ )
        pkn_ApplyGivensRotationf ( c, s, &a[pkn_LowerTrMatIndex(k,j+1)],
                                   &a[pkn_LowerTrMatIndex(i,k)] );
      pkn_ApplySymGivensRotationf ( c, s, &a[pkn_LowerTrMatIndex(j+1,j+1)],
  ...
  do {
    ...
    if ( q-p > 2 ) {
      ...
      pkn_SolveSqEqf ( -0.5*(eigenval[q-2]+eigenval[q-1]),
      ...
      for ( i = p; i < q; i++ )
        eigenval[i] -= mu;
      memcpy ( &dd1[p], &cd[p], (q-p-1)*sizeof(float) );
            /* A -> QR decomposition */
      for ( i = p; i < q-2; i++ ) {
        pkn_FindGivensRotationf ( eigenval[i], cd[i], &c, &s );
        eigenval[i] = c*eigenval[i] - s*cd[i];
        pkn_ApplyGivensRotationf ( c, s, &dd1[i], &eigenval[i+1] );
        dd2[i] = -s*dd1[i+1];  dd1[i+1] *= c;
        cd[i] = c;  sd[i] = s;
      }
      pkn_FindGivensRotationf ( eigenval[q-2], cd[q-2], &c, &s );
      eigenval[q-2] = c*eigenval[q-2] - s*cd[q-2];
      pkn_ApplyGivensRotationf ( c, s, &dd1[q-2], &eigenval[q-1] );
      cd[q-2] = c;  sd[q-2] = s;
            /* computation A = RQ */
      c = cd[p];  s = sd[p];
      for ( i = p; i < q-2; i++ ) {
        pkn_ApplyGivensRotationf ( c, s, &eigenval[i], &dd1[i] );
        cd[i] = -s*eigenval[i+1];  eigenval[i+1] *= c;
        c = cd[i+1];  s = sd[i+1];
        cd[i] = c*dd1[i] - s*dd2[i];
      }
      c = cd[q-2];  s = sd[q-2];
      pkn_ApplyGivensRotationf ( c, s, &eigenval[q-2], &dd1[q-2] );
      cd[q-2] = 0.0;
      pkn_ApplyGivensRotationf ( c, s, &cd[q-2], &eigenval[q-1] );
            /* deflation */
      newblock = false;
      for ( i = p; i < q-1; i++ )
      ...
      pkn_SolveSqEqf ( -0.5*(eigenval[p]+eigenval[p+1]),
