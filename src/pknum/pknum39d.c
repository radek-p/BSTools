                              ...
                              int *qaprof, double **QArows )
{
  ...
  rows1 = pkv_GetScratchMem ( n*sizeof(double*) );
  rows2 = pkv_GetScratchMem ( n*sizeof(double*) );
  if ( !v || !p || !prof1 || !prof2 || !rows1 || !rows2 )
    goto failure;

  memcpy ( prof1, prof, n*sizeof(int) );
  memcpy ( rows1, Arows, n*sizeof(double*) );
  for ( i = 0; i < w; i++ ) {
    ...
    pkn_NRBSymMultd ( n, prof1, rows1[0], rows1, 1, 1, v, 1, p );
    ...
    PKV_MALLOC ( rows2[0], size*sizeof(double) );
    if ( !rows2[0] )
      goto failure;
    if ( !pkn_NRBFindRowsd ( n, prof2, rows2[0], rows2 ) )
      goto failure;
          /* copy the matrix A_{k-1} */
    memset ( rows2[0], 0, size*sizeof(double) );
    for ( j = 0; j < n; j++ )
      memcpy ( &rows2[j][prof1[j]], &rows1[j][prof1[j]],
               (j-prof1[j]+1)*sizeof(double) );
