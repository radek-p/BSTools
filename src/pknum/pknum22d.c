...
boolean pkn_CholeskyDecompd ( int n, double *a )
{
  ...
  for ( j = 0; j < n; j++ ) {
    ...
    a[pkn_SymMatIndex(j,j)] = ljj = sqrt(ljj);
    for ( i = j+1; i < n; i++ ) {
...
{
  int   i, j, i0;
  double aa, *xx, *xxx, *bb, *bbb;

  for ( i = 0, xx = x;  i < n;  i++, xx += xpitch )
    memset ( xx, 0, spdimen*sizeof(double) );
  ...
  for ( i = 0, xx = x, bb = b;
    ...
    for ( j = 0, xxx = x, bbb = b;
      ...
      aa = a[i0+j];
      pkn_AddMatrixMd ( 1, spdimen, 0, xx, 0, bbb, aa, 0, xx );
      pkn_AddMatrixMd ( 1, spdimen, 0, xxx, 0, bb, aa, 0, xxx );
    }
    aa = a[i0+i];
    pkn_AddMatrixMd ( 1, spdimen, 0, xx, 0, bb, aa, 0, xx );
  }
} /*pkn_SymMatrixMultd*/
                              ...
                              int xpitch, double *x )
{
  ...
  for ( i = 0, xx = x;  i < n;  i++, xx += xpitch ) {
    ...
    for ( j = 0, bbb = b;  j <= i;  j++, bbb += bpitch ) {
      ll = l[i0+j];
      pkn_AddMatrixMd ( 1, spdimen, 0, xx, 0, bbb, ll, 0, xx );
    }
  }
                              ...
                              int xpitch, double *x )
{
  ...
  for ( i = 0, xxx = x;  i < n;  i++, xxx += xpitch )
    memset ( xxx, 0, spdimen*sizeof(double) );

  for ( i = 0, bb = b;  i < n;  i++, bb += bpitch ) {
    i0 = pkn_SymMatIndex ( i, 0 );
    for ( j = 0, xxx = x;  j <= i;  j++, xxx += xpitch ) {
      ll = l[i0+j];
      pkn_AddMatrixMd ( 1, spdimen, 0, xxx, 0, bb, ll, 0, xxx );
    }
  }
                               ...
                               int xpitch, double *x )
{
  ...
  for ( i = 0, xx = x;  i < n;  i++, xx += xpitch ) {
    ...
    for ( j = 0, xxx = x;  j < i;  j++, xxx += xpitch ) {
      ll = l[i0+j];
      pkn_AddMatrixMd ( 1, spdimen, 0, xx, 0, xxx, -ll, 0, xx );
    }
    ll = l[i0+i];
    pkn_MultMatrixNumd ( 1, spdimen, 0, xx, 1.0/ll, 0, xx );
  }
                               ...
                               int xpitch, double *x )
{
  ...
  for ( i = n-1, xx = x+((n-1)*xpitch);  i >= 0;  i--, xx -= xpitch ) {
    ...
    for ( j = 0, xxx = x;  j < i;  j++, xxx += xpitch ) {
      ll = l[i0+j];
      pkn_AddMatrixMd ( 1, spdimen, 0, xxx, 0, xx, -ll, 0, xxx );
    }
  }
                            ...
                            int pitch, double *fulla )
{
  ...
  memcpy ( fulla, syma, sizeof(double) );
  for ( i = k = 1, l = pitch;  i < n;  i++, k += i, l += pitch ) {
    memcpy ( &fulla[l], &syma[k], (i+1)*sizeof(double) );
    pkv_Selectd ( i, 1, 1, pitch, &syma[k], &fulla[i] );
  }
                            ...
                            double *syma )
{
  ...
  for ( i = k = l = 0;  i < n;  i++, k += i, l += pitch )
    memcpy ( &syma[k], &fulla[l], (i+1)*sizeof(double) );
                            ...
                            int pitch, double *fulla )
{
  ...
  for ( i = k = l = 0;  i < n-1;  i++, k += i, l += pitch ) {
    memcpy ( &fulla[l], &ltra[k], (i+1)*sizeof(double) );
    memset ( &fulla[l+i+1], 0, (n-i-1)*sizeof(double) );
  }
  memcpy ( &fulla[l], &ltra[k], n*sizeof(double) );
                            ...
                            int pitch, double *fulla )
{
  ...
  memcpy ( fulla, utra, sizeof(double) );
  for ( i = k = 1, l = pitch;  i < n;  i++, k += i, l += pitch ) {
    memset ( &fulla[l], 0, i*sizeof(double) );
    pkv_Selectd ( i+1, 1, 1, pitch, &utra[k], &fulla[i] );
  }
                            ...
                            double *utra )
{
  ...
  for ( i = k = 0;  i < n;  i++, k += i )
    pkv_Selectd ( i+1, 1, pitch, 1, &fulla[i], &utra[k] );
