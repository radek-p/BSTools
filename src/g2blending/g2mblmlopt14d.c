...
printf ( "," );
      ...
      pkv_Tic ( NULL );
#endif
      if ( !pkn_PCGd ( nvars, &cgdata, grad, dcoeff,
...
#endif
      if ( bd2->iHbl ) {
        if ( !g2mbl_MLOptBlockAd ( data, b2 ) )
          goto failure;
      }
      else {
        if ( !g2mbl_MLOptBlockBd ( data, b2 ) )
          goto failure;
      }
...
#endif
      if ( bd1->iHbl ) {
        if ( !g2mbl_MLOptBlockAd ( data, b1 ) )
          goto failure;
      }
      else {
        if ( !g2mbl_MLOptBlockBd ( data, b1 ) )
          goto failure;
      } 
