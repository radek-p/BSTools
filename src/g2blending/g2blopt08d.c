...
typedef struct {
    ...
    int     nkn1, nkn2;
    int     nsq;
    int     nip, nvars, niip, nconstr, neqsw;
    int     hsize;
    double  C;
...
} /*g2bl_ClosedConstrOptLMTDeallocated*/
                   ...
                   void **data )
{
  ...
  int    i, j;
  int    ncp, nsq, nip, niip, nvars, neqsw, hsize, pitch1, pitch2;
  char   *dirtypt, *dirtysq;
  int    *prof;
  double *ftab, *gtab, *htab, *hessian, **hrows;
  ...
  *data = d = NULL;
  ...
  d->nkn2 = nkn2;
        ...
        /* allocate memory */
  size = (niip+nsq)*sizeof(char) + nvars*sizeof(int) + ncp*sizeof(point3d) +
...
printf ( "C = %f\n", d->C );
  ...
  pkv_Selectd ( lastknotu-6, pitch2, pitch2, pitch, coeff, &cp[3].x );
  pkv_Selectd ( 3, pitch2, pitch2, pitch, coeff,
                &cp[(lastknotu-6)*(pitch/3)+3].x );
...
failure:
  g2bl_ClosedConstrOptLMTDeallocated ( data );
  pkv_SetScratchMemTop ( sp );
  return false;
} /*g2bl_ClosedInitBlSurfaceConstrOptLMTd*/

/* ////////////////////////////////////////////////////////////////////////// */
static double _g2bl_ClosedAuxConstrNuFuncd ( int nknots,
                          ...
                          double tC, double *ftab )
{
  ...
  if ( _g2bl_ShiftDecompHessiand ( neqsw, qsize, prof, QA22,
    ...
    pkv_Selectd ( lastknotu-6, pitch2, pitch2, pitch1, coeff, &acp[3].x );
...
boolean g2bl_ClosedIterBlSurfaceConstrOptLMTd ( void *data, boolean *finished )
{
  ...
  double  *hessian, **hrows, **QA11rows, **QA22rows, *QA21,
          *LM22, **LM22rows;
  double  *cmat, *acmat;
  int     lastknotu, lastknotv, pitch, pitch1, pitch2;
  int     nvars, nconstr, neqsw, nsq, nkn, nkn1, hsize;
...
printf ( "%2d: ", d->itn );
  ...
  lco = sqrt ( pkn_ScalarProductd ( pitch1*(lastknotu-3),
               &acp[0].x, &acp[0].x ) );
  pkv_Selectd ( lastknotu-6, pitch2, pitch, pitch2, &cp[3].x, coeff );
  pkn_multiReflectVectord ( nvars, nconstr, cmat, acmat, 1, 1, coeff );
...
switch_to_lmt:
  ...
  g0 = 0.0;
  for ( i = 0; ; i++ ) {
    fg1 = _g2bl_ClosedAuxConstrNuFuncd ( nkn, qcoeff, Nitab, nvars, nconstr,
  ...
  for ( i = 0; gc == gd || fgb <= fgd; i++ ) {
    ...
    if ( fgd <= fgc ) {
      fgb = _g2bl_ClosedAuxConstrNuFuncd ( nkn, qcoeff, Nitab, nvars, nconstr,
                     cmat, acmat, qsize, qa22prof, QA22rows[0],
...
/*      gc = (1.0-TAU)*ga + TAU*gd; */
      gc = GRDIV ( ga, gd );
      fgc = _g2bl_ClosedAuxConstrNuFuncd ( nkn, qcoeff, Nitab, nvars, nconstr,
                     cmat, acmat, qsize, qa22prof, QA22rows[0],
...
/*      gd = TAU*gc + (1.0-TAU)*gb; */
      gc = GRDIV ( gb, gc );
      fgd = _g2bl_ClosedAuxConstrNuFuncd ( nkn, qcoeff, Nitab, nvars, nconstr,
                     cmat, acmat, qsize, qa22prof, QA22rows[0],
  ...
  if ( d->newpoint ) {
    pkv_Selectd ( lastknotu-6, pitch2, pitch2, pitch, mcoeff, &cp[3] );
    pkv_Selectd ( 3, pitch2, pitch2, pitch, mcoeff,
                  &cp[(lastknotu-6)*(pitch/3)+3] );
  }
...
failure:
  ...
  pkv_SetScratchMemTop ( sp );
  return false;
} /*g2bl_ClosedIterBlSurfaceConstrOptLMTd*/
                   ...
                   int maxit, int nkn1, int nkn2 )
{
  ...
  data = NULL;
  if ( !g2bl_ClosedInitBlSurfaceConstrOptLMTd ( lastknotu, lastknotv, pitch, cp,
  ...
  for ( itn = 0; itn < maxit; itn++ ) {
    if ( !g2bl_ClosedIterBlSurfaceConstrOptLMTd ( data, &finished ) )
      goto failure;
    if ( finished )
      break;
  }
...
#endif
  g2bl_ClosedConstrOptLMTDeallocated ( (void**)&data );
  pkv_SetScratchMemTop ( sp );
  return true;
...
failure:
  ...
  pkv_SetScratchMemTop ( sp );
  return false;
} /*g2bl_ClosedFindBlSurfaceConstrLMTd*/
