                                    ...
                                    void **data )
{
  ...
  *data = d = NULL;
  if ( lastknotu < 10 || lastknotv < 10 ) {
    pkv_SignalError ( LIB_G2BLENDING, 14, ERRMSG_2 );
    goto failure;
  }
  if ( nkn1 < 3 || nkn1 > 10 || nkn2 < nkn1 || nkn2 > 10 ) {
    pkv_SignalError ( LIB_G2BLENDING, 15, ERRMSG_2 );
    goto failure;
  }
  ...
  d = *data;
  if ( !d ) {
    pkv_SignalError ( LIB_G2BLENDING, 16, ERRMSG_1 );
    goto failure;
  }
  ...
  if ( !dirtypt ) {
    pkv_SignalError ( LIB_G2BLENDING, 17, ERRMSG_1 );
    goto failure;
  }
  d->dirtypt = dirtypt;
...
printf ( "nkn1 = %d, nkn2 = %d\n", nkn1, nkn2 );
  ...
  if ( !ftab ) {
    pkv_SignalError ( LIB_G2BLENDING, 18, ERRMSG_1 );
    goto failure;
  }
  d->ftab = ftab;
  ...
  s4 = g2bl_NiSize ( nkn2 );
  size = (nkn1 + s1 + s2 + s3 + s4 + nkn2)*sizeof(double);
  PKV_MALLOC ( aqcoeff, size );
  if ( !aqcoeff ) {
    pkv_SignalError ( LIB_G2BLENDING, 19, ERRMSG_1 );
    goto failure;
  }
  d->aqcoeff = aqcoeff;
  ...
  if ( !_g2bl_TabBasisFuncd ( nkn1, &aqknots, &aqcoeff,
    ...
    pkv_SignalError ( LIB_G2BLENDING, 20, ERRMSG_3 );
    goto failure;
  }
  ...
  if ( !_g2bl_TabBasisFuncd ( nkn2, &bqknots, &bqcoeff,
    ...
    pkv_SignalError ( LIB_G2BLENDING, 21, ERRMSG_3 );
    goto failure;
  }
...
boolean g2bl_IterBlSurfaceOptLMTd ( void *data, boolean *finished )
{
  ...
  int         nkn, nkn1, nvars, nip, nsq, hsize;
...
#define MLFUNC(nu) \
_g2bl_AuxNuFuncd ( nkn, qcoeff, Nitab, nvars, hsize, prof, hessian, \
...
printf ( "%2d: ", d->itn );
  ...
  if ( !acp || !grad || !coeff || !dcoeff ) {
    pkv_SignalError ( LIB_G2BLENDING, 22, ERRMSG_0 );
    goto failure;
  }
...
printf ( "+" );
    ...
    pkn_SubtractMatrixd ( lastknotu-9, pitch2, pitch, &cp[pitch+3].x,
                          pitch2, dcoeff,
                          pitch1, &acp[pitch1+3].x );
...
printf ( "," );
        ...
        pkn_NRBLowerTrSolved ( nvars, prof, Lhessian, Lhrows, 1, 1, grad, 1, dcoeff );
        pkn_NRBUpperTrSolved ( nvars, prof, Lhessian, Lhrows, 1, 1, dcoeff, 1, dcoeff );
        pkn_SubtractMatrixd ( lastknotu-9, pitch2,
                              pitch, &cp[pitch+3].x, pitch2, dcoeff,
                              pitch1, &acp[pitch1+3].x );
#ifdef _DEBUG
printf ( "G" );
#endif
...
switch_to_lmt:
  ...
  g0 = 0.0;
  for ( i = 0; ; i++ ) {
    fg1 = _g2bl_AuxNuFuncd ( nkn, qcoeff, Nitab, nvars, hsize, prof, hessian,
  ...
  for ( i = 0; gc == gd || fgb <= fgd; i++ ) {
    ...
    if ( fgd <= fgc ) {
      fgb = _g2bl_AuxNuFuncd ( nkn, qcoeff, Nitab, nvars, hsize, prof, hessian,
...
/*
    if ( fgc < fgd ) {
      ...
      gd = gc;  fgd = fgc;
      gc = GRDIV ( ga, gd );
      fgc = _g2bl_AuxNuFuncd ( nkn, qcoeff, Nitab, nvars, hsize, prof, hessian,
      ...
      gc = gd;  fgc = fgd;
      gd = GRDIV ( gb, gc );
      fgd = _g2bl_AuxNuFuncd ( nkn, qcoeff, Nitab, nvars, hsize, prof, hessian,
...
*/
  ...
  pkn_SubtractMatrixd ( lastknotu-9, pitch2, pitch, &cp[pitch+3].x,
                        pitch2, coeff, pitch, &cp[pitch+3].x );
                                 ...
                                 int maxit, int nkn1, int nkn2 )
{
  ...
  data = NULL;
  if ( !g2bl_InitBlSurfaceOptLMTd ( lastknotu, lastknotv, pitch, cp,
                                    C, dO, dM, nkn1, nkn2, (void**)&data ) )
    goto failure;

  finished = false;
  for ( itn = 0; itn < maxit; itn++ ) {
    if ( !g2bl_IterBlSurfaceOptLMTd ( data, &finished ) )
      goto failure;
    if ( finished )
      break;
  }
