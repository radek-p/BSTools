...
/* ///////////////////////////////////////////////////////////////////////// */
static double _g2mbl_MLSAuxNuFuncd ( int nkn, double *qcoeff, double **Nitabs,
...
printf ( "F" );
#endif
  if ( _g2bl_ShiftDecompHessiand ( nvcp, hsize, hprof,
                                   hrows[0], lhrows[0], lhrows, nu ) ) {
    pkn_NRBLowerTrSolved ( nvcp, hprof, lhrows[0], lhrows, 1, 1, grad, 1, dcoeff );
    pkn_NRBUpperTrSolved ( nvcp, hprof, lhrows[0], lhrows, 1, 1, dcoeff, 1, dcoeff );
    _g2mbl_MLSAddCPIncrement ( nvcp, vncpi, mvcp, mvcpn, dcoeff, auxmvcp );
    return g2mbl_MLFuncd ( nkn, qcoeff, Nitabs, Jac, nv, auxmvcp,
...
printf ( "." );
#endif
    return 1.5*MYINFINITY;
  }
} /*_g2mbl_MLSAuxNuFuncd*/
...
#define MLFUNC(nu) \
_g2mbl_MLSAuxNuFuncd ( nkn, qcoeff, Nitabs, Jac, nv, mvcp, mvcpn, nvcp, vncpi, \
    ...
    if ( !(bd->fghflag & FLAG_H) ) {
      if ( bl > 0 && !(bd->fghflag & FLAG_ADVANCE) ) {
        ...
        if ( !bd1->iHbl || !(bd1->fghflag & FLAG_H) )
          goto comp_Hessian;
        if ( !g2mbl_MLSGetHessianRowsd ( nv, bd1->nvcp, bd1->vncpi,
...
printf ( " H" );
        ...
        pkv_Tic ( NULL );
#endif
        if ( !g2mbl_MLSFuncGradHessianBd ( d->nkn1, d->aqcoeff,
...
printf ( "G" );
    ...
    if ( !(bd->fghflag & FLAG_LH) ) {
      ...
      if ( pkn_NRBSymCholeskyDecompd ( nvcp, hprof, lhrows[0], lhrows, NULL ) )
        bd->fghflag |= FLAG_LH;
      else {
#ifdef _DEBUG
printf ( "!" );
...
lm_trajectory:
    ...
    if ( d->nu[0] <= 0.0 ) {
      if ( !pkn_NRBSymFindEigenvalueIntervald ( nvcp, hprof, hrows[0], hrows,
                                                &lmin, &lmax ) ) {
printf ( "%s\n", ERRMSG_23 );
        goto failure;
      }
