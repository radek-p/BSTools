...
boolean _g2mbl_CMPSSetupBlockCGPrecondd ( mesh_ml_optdata *d, int bn )
{
  ...
  PKV_MALLOC ( bd->hprof, bnwcp*sizeof(int) );
  if ( !bd->hprof )
    goto failure;
...
failure:
  pkv_SetScratchMemTop ( sp );
  return false;
} /*_g2mbl_CMPSSetupBlockCGPrecondd*/
...
printf ( "%s\n", ERRMSG_21 );
      ...
      if ( d->bd[2*i+1].nvcp > MAX_NVCPS ) {
        if ( !_g2mbl_CMPSSetupBlockCGPrecondd ( d, i ) )
...
printf ( "%s\n", ERRMSG_20 );
  ...
  return true;
} /*_g2mbl_CMPSSetupBlockHessiansd*/
