...
/* the procedure g2bl_ClosedFindBlSurfaceConstrLMTd, i.e. the unknown      */
/* variables are the coordinates of the internal control points and the    */
/* right hand side matrix has one column.                                  */
                                          ...
                                          int *nconstr, double *cmat, double *crhs )
{
  ...
  if ( !acmat )
    goto failure;
  if ( !g2bl_SetupClosedULConstraintsd ( lastknotu, lastknotv, 3,
                                         ppitch, (double*)cp,
...
failure:
  pkv_SetScratchMemTop ( sp );
  return false;
} /*g2bl_SetupClosedUNLConstraintsd*/
