...
boolean xge_Slidebar2dMsg ( xge_widget *er, int msg, int key, short x, short y )
{
  ...
  x0 = er->x + 5 + (int)((*slipos0)*(double)(er->w - 10));
  x1 = er->x + 5 + (int)((*slipos1)*(double)(er->w - 10));
  inval = false;
  switch ( er->state ) {
case xgestate_NOTHING:
    if ( msg == xgemsg_MCLICK ) {
      if ( key & xgemouse_LBUTTON_DOWN ) {
        ...
        z0 = mz - 1.0/(double)(er->w-10) - 0.5*dz;
        z0 = max ( z0, 0.0 );
        z1 = z0 + dz;
        z = 0.5*(z0+z1);
        goto update2;
        ...
        z1 = mz + 1.0/(double)(er->w-10) + 0.5*dz;
        z1 = min ( z1, 1.0 );
        z0 = z1 - dz;
        z = 0.5*(z0+z1);
        goto update2;
                                ...
                                double *data )
{
  return xge_NewWidget ( window_num, prev, id, w, h, x, y, data, NULL,
                         xge_Slidebar2dMsg, xge_DrawSlidebar2d );
} /*xge_NewSlidebar2d*/
