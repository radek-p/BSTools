...
static void _xge_QuatRotBalldDrawSpecial ( xge_widget *er, boolean onscreen )
{
  ...
  Trans3dTof ( qball->tr, &tr );
  _xge_QuatRotBallDrawCircles ( xc, yc, R, &tr, xge_OutPixels );
  if ( onscreen )
    xgeCopyRectOnScreen ( er->w, er->h, er->x, er->y );
} /*_xge_QuatRotBalldDrawSpecial*/

#ifdef USE_XEXT_SHAPE
static void _xge_QuatRotBalldMaskSpecialWin ( xge_widget *wdg )
{
  xge_quatrotballd *qball;
  short            R;
  trans3f          tr;

  qball = (xge_quatrotballd*)wdg->data0;
  R = qball->R;
  Trans3dTof ( qball->tr, &tr );
  ...
  _xge_QuatRotBallDrawCircles ( R, R, R, &tr, _xge_OutSpecialMaskPixels );
} /*_xge_QuatRotBalldMaskSpecialWin*/
#endif
