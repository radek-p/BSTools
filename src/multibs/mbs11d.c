...
/* Create a working area. */
  ...
  if ( !auxcp || !akn ) {
    pkv_SignalError ( LIB_MULTIBS, 11, ERRMSG_0 );
    exit ( 1 );
  }
...
/* executed, all arcs of one curve are processed.                      */
  if ( !inctlpoints )
    inctlpoints = outctlpoints;
  mbs_multiMaxKnotInsd ( ncurves, spdimen, indegree, inlastknot, inknots,
                         inpitch, inctlpoints, &lkn, akn, auxpitch, auxcp,
                         &skipl, &skipr );
  auxcp += skipl*spdimen;
  lkn -= skipl+skipr;
  for ( k = 0; k < ncurves; k++ )
    mbs_multiBCDegElevd ( ki, spdimen, (indegree+1)*spdimen, indegree, NULL,
