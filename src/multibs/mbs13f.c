                        ...
                        float t, int *fnz, int *nnz, float *bfv )
{
  ...
  k = mbs_FindKnotIntervalf ( -1, lastknot, knots, t, &r );
  ...
  if ( k < degree ) {
    ...
    auxkn = pkv_GetScratchMemf ( auxsize = 3*(degree+1) );
    auxbfv = &auxkn[2*(degree+1)];
    l = degree-k;
    memcpy ( &auxkn[l], knots, (2*(degree+1)-l)*sizeof(float) );
    for ( i = 0; i < l; i++ )
      auxkn[i] = knots[0];
    mbs_deBoorBasisf ( degree, 2*degree+1, auxkn, t, fnz, nnz, auxbfv );
    if ( *nnz > l ) {
      *nnz -= l;
      memcpy ( bfv, &auxbfv[l], (*nnz)*sizeof(float) );
    }
    else
      *nnz = 0;
    *fnz = 0;
    pkv_FreeScratchMemf ( auxsize );
  }
    ...
    memcpy ( auxkn, &knots[k-degree], (2*(degree+1)-l)*sizeof(float) );
    ...
    *fnz = k-degree;
    ...
    if ( *nnz > 0 )
      memcpy ( bfv, auxbfv, (*nnz)*sizeof(float) );
    pkv_FreeScratchMemf ( auxsize );
  }
  else {
    ...
    *nnz = degree+1-r;
    ...
    for ( j = 1; j <= degree; j++ ) {
      ...
      bfv[k-l] = (float)((1.0-beta)*bfv[k-l]);
    }
  }
} /*mbs_deBoorBasisf*/
