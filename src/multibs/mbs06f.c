                        ...
                        float *ctlpoints, float t )
{
  ...
  if ( mp > 0 )
    pkv_Movef ( ncurves, mp*spdimen, outpitch, spdimen,
  ...
  for ( i = k-r, is = i*spdimen;
    ...
    for ( l = 0, ll = 0; l < ncurves; l++, ll += outpitch )
      for ( j = 0; j < spdimen; j++ )
        ctlpoints[ll+is+j] =
            (float)(beta*ctlpoints[ll+is-spdimen+j] + alpha*ctlpoints[ll+is+j]);
  }
  if ( nkn > k+1 )
    memmove ( &knots[k+2], &knots[k+1], (nkn-k)*sizeof(float) );
  knots[k+1] = t;
  (*lastknot)++;
  return k;
} /*mbs_multiKnotInsf*/
