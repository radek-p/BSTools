...
static void _mbs_multiMultScaledBezd ( int nproducts, int spdimen,
                                       int degpoly, int polypitch,
                                       const double *polycoef,
                                       int degcurve, int curvepitch,
                                       const double *curvecoef,
                                       int outpitch, double *outcoef )
{
  int   i, j, k, l, outdeg;
  double p;
  ...
  for ( k = 0;
    ...
    memset ( outcoef, 0, (outdeg+1)*spdimen*sizeof(double) );
    for ( i = 0; i <= degpoly; i++ ) {
      p = polycoef[i];
...
void mbs_multiMultBezCd ( int nscf, int degscf, int scfpitch,
...
/* Allocate working memory area and copy data. */
  ...
  pkv_Selectd ( nvecf, (degvecf+1)*spdimen, vecfpitch,
                (degvecf+1)*spdimen, vecfcp, auxvecf );

/* Convert data to scaled Bernstein bases */
  mbs_multiBezScaled ( degscf, nscf, 1, 1, degscf+1, auxscf );
  mbs_multiBezScaled ( degvecf, nvecf, 1, spdimen, (degvecf+1)*spdimen,
                       auxvecf );

/* Do the actual multiplication and convert the result to Bernstein basis. */
  _mbs_multiMultScaledBezd ( nprod, spdimen, degscf, degscf+1, auxscf,
                             degvecf, (degvecf+1)*spdimen, auxvecf,
                             prodpitch, prodcp );
  *degprod = degscf+degvecf;
  mbs_multiBezUnscaled ( *degprod, 1, nprod, spdimen, prodpitch, prodcp );
...
/*    prodlastknot = mbs_BSProdRepSized ( degscf, scflastknot, scfknots,    */
...
/* computation.                                                             */
void mbs_multiMultBSCd ( int nscf, int degscf,
                         ...
                         int prodpitch, double *prodcp )
{
  ...
  int   i, j;
  double *auxkn, *sauxc, *vauxc, *akn, *acp, *auxprod, *auxpr, *aakn;
...
/* amount of scratch memory needed for intermediate results.          */
  mbs_SetBSProdKnotsd ( degscf, scflastknot, scfknots,
                        degvecf, vecflastknot, vecfknots,
  ...
  *degprod = degpr = degscf+degvecf;
  npp = mbs_NumKnotIntervalsd ( degpr, lastpknot, prodknots );
...
/* may be represented by knots of multiplicity greater than degree+1.       */
  akn = pkv_GetScratchMemd ( max(scflastknot,vecflastknot)+1 );
  ...
  memcpy ( akn, scfknots, (scflastknot+1)*sizeof(double) );
  pkv_Selectd ( nscf, scflastknot-degscf, scfpitch, scflastknot-degscf,
                scfcoef, acp );
  if ( nscf > 1 )
    scfpitch = scflastknot-degscf;
  else
    scfpitch = 0;
  mbs_multiRemoveSuperfluousKnotsd ( nscf, 1, degscf, &scflastknot,
...
/* Now do the same thing with the vector argument of multiplication. */
  memcpy ( akn, vecfknots, (vecflastknot+1)*sizeof(double) );
  pkv_Selectd ( nvecf, (vecflastknot-degvecf)*spdimen, vecfpitch,
                (vecflastknot-degvecf)*spdimen, vecfcp, acp );
  if ( nvecf > 1 )
    vecfpitch = (vecflastknot-degvecf)*spdimen;
  else
    vecfpitch = 0;
  mbs_multiRemoveSuperfluousKnotsd ( nvecf, spdimen, degvecf, &vecflastknot,
...
/* bases and followed by conversion of the product to Bernstein basis.       */
  mbs_multiBezScaled ( degscf, npp, nscf, 1, scfpitch, sauxc );
  ...
  for ( i = 0, auxpr = auxprod;
    ...
    _mbs_multiMultScaledBezd ( npp, spdimen, degscf, degscf+1, sauxc,
                               degvecf, (degvecf+1)*spdimen, vauxc,
                               (degpr+1)*spdimen, auxpr );
  mbs_multiBezUnscaled ( degpr, npp, nprod, spdimen,
...
/* of the Oslo algorithm. */
  mbs_SetKnotPatternd ( lastpknot-2, &prodknots[1], degpr+1,
                        &auxlastkn, auxkn );
  auxkn[0] = prodknots[0];
