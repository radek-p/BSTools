...
/* compute the total size of data and opcodes */
  ...
  for ( i = 0; i < nelem; i++ ) {
    ...
    if ( (knots = bound[i].knots) ) {  /* a B-spline curve */
      kn = mbs_NumKnotIntervalsd ( deg, N, knots );
      size += kn*(dim*(deg+1)*sizeof(double) + 2 + sizeof(short));
    }
    else if ( N == -1 )              /* a Bezier curve */
      size += dim*(deg+1)*sizeof(double) + 2 + sizeof(short);
    else if ( deg == 1 && N > 0 )    /* a polyline */
      size += dim*(N+1)*sizeof(double) + 2 + sizeof(short);
    else
      return 0;
  }
  return size + 1;
} /*mbs_TrimCVBoundSized*/
                                   ...
                                   void *buffer )
{
  ...
  for ( i = 0; i < nelem; i++ ) {
    ...
    if ( (knots = bound[i].knots) ) {  /* a B-spline curve */
      ...
      inpitch  = dim*(deg+1)*sizeof(double);
      ...
      bufp[1] = (char)dim;
      *bn = (short)deg;
      memcpy ( bpoints, points, dim*(deg+1)*sizeof(double) );
      bufp += 2 + sizeof(short) + dim*(deg+1)*sizeof(double);
    }
    else if ( deg == 1 && N >0 ) {  /* a polyline */
      if ( bound[i].closing )
        bufp[0] = 1;
