                                 ...
                                 point4f *cp, float eps, Box3f *bbox )
{
  ...
  if ( ncp & 0x01 ) {
    ...
    Point4to3f ( &q[1], &b );
    pkv_Sort2f ( &a.x, &b.x );  bbox->x0 = a.x;  bbox->x1 = b.x;
    pkv_Sort2f ( &a.y, &b.y );  bbox->y0 = a.y;  bbox->y1 = b.y;
    pkv_Sort2f ( &a.z, &b.z );  bbox->z0 = a.z;  bbox->z1 = b.z;
    j = k = 2;
  }
  while ( k < ncp ) {
    if ( j >= ncols ) { i++;  q = &cp[i*pitch];  Point4to3f ( &q[0], &a );  j = 1; }
                 else Point4to3f ( &q[j++], &a );
    if ( j >= ncols ) { i++;  q = &cp[i*pitch];  Point4to3f ( &q[0], &b );  j = 1; }
                 else Point4to3f ( &q[j++], &b );
    pkv_Sort2f ( &a.x, &b.x );
