...
static boolean AnalyzePartitiond ( GHoleDomaind *domain )
{
  ...
  if ( _gh_AnalyzePartitiond ( domain, G2H_OMCDEG, privateG2->omcbc,
...
static void ComputeAuxiMat ( GHoleDomaind *domain )
{
  G2HolePrivateRecd *privateG2;
  int      i, hole_k;
  vector2d diu, div, diuu, diuv, divv, diuuu, diuuv, diuvv,
           diuuuu, diuuuv, diuuvv;
  vector2d *omcbc, *omcbcd, *omcbcdd;
  ...
  for ( i = 0; i < hole_k; i++ ) {
    ...
    pkn_Setup2DerA11Matrixd ( diu.x, diu.y, div.x, div.y, A11 );
    pkn_Setup2DerA21Matrixd ( diuu.x, diuu.y, diuv.x,
                              diuv.y, divv.x, divv.y, A21 );
    pkn_Setup2DerA22Matrixd ( diu.x, diu.y, div.x, div.y, A22 );
    pkn_Setup2DerA31Matrixd ( diuuu.x, diuuu.y, diuuv.y, diuuv.y,
                              diuvv.x, diuvv.y, 0.0, 0.0, A31 );
    pkn_Setup2DerA32Matrixd ( diu.x, diu.y, div.x, div.y,
                              diuu.x, diuu.y, diuv.x,
                              diuv.y, divv.x, divv.y, A32 );
    pkn_Setup2DerA33Matrixd ( diu.x, diu.y, div.x, div.y, A33 );
    pkn_Setup2DerA41Matrixd ( diuuuu.x, diuuuu.y, diuuuv.x, diuuuv.y,
                              diuuvv.x, diuuvv.y, 0.0, 0.0, 0.0, 0.0, A41 );
    pkn_Setup2DerA42Matrixd ( diu.x, diu.y, div.x, div.y,
                              diuu.x, diuu.y, diuv.x,
                              diuv.y, divv.x, divv.y,
                              diuuu.x, diuuu.y, diuuv.y, diuuv.y,
                              diuvv.x, diuvv.y, 0.0, 0.0, A42 );
    pkn_Setup2DerA43Matrixd ( diu.x, diu.y, div.x, div.y,
                              diuu.x, diuu.y, diuv.x,
                              diuv.y, divv.x, divv.y, A43 );
    pkn_Setup2DerA44Matrixd ( diu.x, diu.y, div.x, div.y, A44 );
  }
} /*ComputeAuxiMat*/

/* ///////////////////////////////////////////////////////////////////////// */
static void FindDivDiff4Coeff ( double a, double b, double c, double d, double e,
                                double *dd )
{
  ...
  dd[0] = (double)(1.0/((a-b)*(a-c)*(a-d)*(a-e)));
  dd[1] = (double)(1.0/((b-a)*(b-c)*(b-d)*(b-e)));
  dd[2] = (double)(1.0/((c-a)*(c-b)*(c-d)*(c-e)));
  dd[3] = (double)(1.0/((d-a)*(d-b)*(d-c)*(d-e)));
  dd[4] = (double)(1.0/((e-a)*(e-b)*(e-c)*(e-d)));
} /*FindDivDiff4Coeff*/

static void FindDivDiff5Coeff1 ( double a, double b, double c,
                                 double *dd )
{
  ...
  ba = (double)(1.0/(b-a));  ca = (double)(1.0/(c-a));  cb = (double)(1.0/(c-b));

  dd[1] = ba*ba*ca*ca;
  dd[0] = (double)(2.0*(ca+ba)*dd[1]);
  dd[3] = cb*cb*ba*ba;
  dd[2] = (double)(2.0*(cb-ba)*dd[3]);
  dd[5] = cb*cb*ca*ca;
  dd[4] = (double)(-2.0*(cb+ca)*dd[5]);
} /*FindDivDiff5Coeff1*/

static void FindDivDiff5Coeff2 ( double a, double b, double c, double d,
                                 double *dd )
{
    /* compute the coefficients of the divided difference of 5th order  */
  ...
  double ba, ca, cb, da, db, dc;

  ba = (double)(1.0/(b-a));  ca = (double)(1.0/(c-a));  cb = (double)(1.0/(c-b));
  da = (double)(1.0/(d-a));  db = (double)(1.0/(d-b));  dc = (double)(1.0/(d-c));

  dd[0] = -ba*ba*ca*ca*da;
  dd[1] = (double)(-(db+2.0*cb-ba)*ba*db*cb*cb);
  dd[2] = -ba*db*cb*cb;
  dd[3] = (double)(-(dc-2.0*cb-ca)*dc*ca*cb*cb);
  dd[4] = -dc*ca*cb*cb;
  dd[5] = da*db*db*dc*dc;
} /*FindDivDiff5Coeff2*/

static void ReparHomoPoly1Der ( double *p1, int i, double *AuxiMat,
                                double *br0bc, double *br0cr1bc, double *br0cr2bc )
{
  double q1[2], q2[3], q3[4], q4[5];
  double *A11, *A21, *A31, *A41;

  A11 = &AuxiMat[i*125];  A21 = &A11[4];  A31 = &A21[6+9];  A41 = &A31[8+12+16];
  pkn_MultMatrixd ( 2, 2, 2, A11, 1, 1, p1, 1, q1 );
  pkn_MultMatrixd ( 3, 2, 2, A21, 1, 1, p1, 1, q2 );
  pkn_MultMatrixd ( 4, 2, 2, A31, 1, 1, p1, 1, q3 );
  pkn_MultMatrixd ( 5, 2, 2, A41, 1, 1, p1, 1, q4 );
...
static void ReparHomoPoly2Der ( double *p2, int i, double *AuxiMat,
                                double *br0bc, double *br0cr1bc, double *br0cr2bc )
{
  double q2[3], q3[4], q4[5];
  double *A22, *A32, *A42;

  A22 = &AuxiMat[i*125+4+6];  A32 = &A22[9+8];  A42 = &A32[12+16+10];
  pkn_MultMatrixd ( 3, 3, 3, A22, 1, 1, p2, 1, q2 );
  pkn_MultMatrixd ( 4, 3, 3, A32, 1, 1, p2, 1, q3 );
  pkn_MultMatrixd ( 5, 3, 3, A42, 1, 1, p2, 1, q4 );
  br0bc[(G2H_OMCDEG+1)*i+2]    = q2[0];
  br0bc[(G2H_OMCDEG+1)*i+3]    = q3[0];
  br0bc[(G2H_OMCDEG+1)*i+4]    = q4[0];
  br0cr1bc[G2H_OMCDEG*i+1] = q2[1];
...
static void ReparHomoPoly3Der ( double *p3, int i, double *AuxiMat,
                                double *br0bc, double *br0cr1bc, double *br0cr2bc,
                                char sw )
{
  double q3[4], q4[5];
  ...
  pkn_MultMatrixd ( 4, 4, 4, A33, 1, 1, p3, 1, q3 );
  pkn_MultMatrixd ( 5, 4, 4, A43, 1, 1, p3, 1, q4 );
  switch ( sw ) {
case 0:
    br0bc[(G2H_OMCDEG+1)*i+3]    = q3[0];
    br0bc[(G2H_OMCDEG+1)*i+4]    = q4[0];
...
static void ReparHomoPoly4Der ( double *p4, int i, double *AuxiMat,
                                double *br0bc, double *br0cr1bc, double *br0cr2bc,
                                char sw )
{
  double q4[5];
  ...
  pkn_MultMatrixd ( 5, 5, 5, A44, 1, 1, p4, 1, q4 );
  switch ( sw ) {
case 0:
    br0bc[(G2H_OMCDEG+1)*i+4]    = q4[0];
    br0cr1bc[G2H_OMCDEG*i+3] = q4[1];
...
#define TOL 1.0e-6
  ...
  double   *br0bc, *br0cr1bc, *br0cr2bc;
  double   p1[2], p2[3], p3[4], p4[5];
  double   alpha0, sa0, ca0, sa, sb, ca, cb, a, b;
  double   u, n, m, normf;
  double   ddc[6];
  int      sfn;
  int      i, j, k;
  ...
  br0bc = pkv_GetScratchMemd ( 3*G2H_OMCDEG*hole_k );
  if ( !br0bc )
    goto failure;

  br0cr1bc = &br0bc[hole_k*(G2H_OMCDEG+1)];   br0cr2bc = &br0cr1bc[hole_k*G2H_OMCDEG];

  memset ( br0bc, 0, hole_k*3*G2_CROSS00DEG*sizeof(double) );
  switch ( fn ) {
case 0:    /* f(x,y) = 1 */
    for ( i = 0; i < hole_k; i++ )
...
default:   /* splines */
    ...
    if ( sfn < spdimen[0] ) {                 /* a cubic half-polynomial */
      ...
      sa = (double)sin ( spartition[j].malpha );
      ca = (double)cos ( spartition[j].malpha );
      p3[0] = (double)(-6.0*sa*sa*sa);  p3[1] = (double)(6.0*sa*sa*ca);
      p3[2] = (double)(-6.0*sa*ca*ca);  p3[3] = (double)(6.0*ca*ca*ca);
      for ( i = 0; i < hole_k; i++ ) {
        sb = (double)sin ( partition[i] );  cb = (double)cos ( partition[i] );
        if ( sa*cb-ca*sb > TOL )
          ReparHomoPoly3Der ( p3, i, privateG2->AuxiMat,
                              br0bc, br0cr1bc, br0cr2bc, 0 );
      }
      ...
      sfn -= spdimen[0];
      sa0 = (double)sin ( alpha0 );  ca0 = (double)cos ( alpha0 );
      ...
      for ( j = 0; j < 5; j++ ) {
        sa = (double)sin ( spartition[sfn+4-j].malpha );
        ca = (double)cos ( spartition[sfn+4-j].malpha );
        ...
        p3[0] = (double)(6.0*ddc[j]*a*a*a);
        p3[1] = (double)(6.0*ddc[j]*a*a*b);
        p3[2] = (double)(6.0*ddc[j]*a*b*b);
        p3[3] = (double)(6.0*ddc[j]*b*b*b);
      ...
      ca = (double)cos ( spartition[j].malpha );
      if ( !(sfn & 0x0001) ) {
        p4[0] = (double)(24.0*sa*sa*sa*sa);  p4[1] = (double)(-24.0*ca*sa*sa*sa);
        p4[2] = (double)(24.0*ca*ca*sa*sa);  p4[3] = (double)(-24.0*ca*ca*ca*sa);
        p4[4] = (double)(24.0*ca*ca*ca*ca);
      }
      else {
        p4[0] = (double)(-24.0*ca*sa*sa*sa);
        p4[1] = (double)( 6.0*sa*sa*(3.0*ca*ca-sa*sa));
        p4[2] = (double)( 12.0*ca*sa*(sa*sa-ca*ca));
        p4[3] = (double)( 6.0*ca*ca*(ca*ca-3.0*sa*sa));
        p4[4] = (double)( 24.0*ca*ca*ca*sa);
      }
      for ( i = 0; i < hole_k; i++ ) {
        sb = (double)sin ( partition[i] );  cb = (double)cos ( partition[i] );
        if ( sa*cb-ca*sb > 0.0 )
      ...
      if ( !(sfn & 0x0001) ) {
        ...
        for ( j = 0; j < 3; j++ ) {
          sa = (double)sin ( spartition[sfn+2-j].malpha );
          ca = (double)cos ( spartition[sfn+2-j].malpha );
          ...
          p4[0] = (double)(24.0*ddc[j+j]*a*a*a*a - 96.0*ddc[j+j+1]*a*a*a*ca0);
          p4[1] = (double)(24.0*ddc[j+j]*a*a*a*b - 24.0*ddc[j+j+1]*a*a*(a*sa0+3.0*b*ca0));
          p4[2] = (double)(24.0*ddc[j+j]*a*a*b*b - 48.0*ddc[j+j+1]*a*b*(a*sa0+b*ca0));
          p4[3] = (double)(24.0*ddc[j+j]*a*b*b*b - 24.0*ddc[j+j+1]*b*b*(3.0*a*sa0+b*ca0));
          p4[4] = (double)(24.0*ddc[j+j]*b*b*b*b - 96.0*ddc[j+j+1]*b*b*b*sa0);
        ...
        for ( j = 0; j < 4; j++ ) {
          sa = (double)sin ( spartition[sfn+3-j].malpha );
          ca = (double)cos ( spartition[sfn+3-j].malpha );
          u = spartition[sfn+3-j].knot;  a = sa0-u*ca0;  b = -(ca0+u*sa0);
          switch ( j ) {
        case 0: k = 0;  goto SINGLE;
        case 1: k = 1;  goto DOUBLE;
        case 2: k = 3;
DOUBLE:     p4[0] = (double)(24.0*ddc[k]*a*a*a*a - 96.0*ddc[k+1]*a*a*a*ca0);
            p4[1] = (double)(24.0*ddc[k]*a*a*a*b - 24.0*ddc[k+1]*a*a*(a*sa0+3.0*b*ca0));
            p4[2] = (double)(24.0*ddc[k]*a*a*b*b - 48.0*ddc[k+1]*a*b*(a*sa0+b*ca0));
            p4[3] = (double)(24.0*ddc[k]*a*b*b*b - 24.0*ddc[k+1]*b*b*(3.0*a*sa0+b*ca0));
            p4[4] = (double)(24.0*ddc[k]*b*b*b*b - 96.0*ddc[k+1]*b*b*b*sa0);
            break;
        case 3: k = 5;
SINGLE:     p4[0] = (double)(24.0*ddc[k]*a*a*a*a);
            p4[1] = (double)(24.0*ddc[k]*a*a*a*b);
            p4[2] = (double)(24.0*ddc[k]*a*a*b*b);
            p4[3] = (double)(24.0*ddc[k]*a*b*b*b);
            p4[4] = (double)(24.0*ddc[k]*b*b*b*b);
            break;
          } /* switch */
          for ( i = 0; i < hole_k; i++ ) {
            sb = (double)sin ( partition[i] );  cb = (double)cos ( partition[i] );
...
static boolean GetABasisFunctiond ( GHoleDomaind *domain, int fn,
                                    double *bbr0,
                                    double *bbr0cr1, double *bbq0cr1,
                                    double *bbr0cr2, double *bbq0cr2 )
{
#define TOL 1.0e-6
  void     *sp;
  G2HolePrivateRecd *privateG2;
  int      hole_k;
  double   *br0cr1, *br0cr2;
  ...
  br0cr1 = pkv_GetScratchMemd ( (2*G2H_OMCDEG-1)*hole_k );
  if ( !br0cr1 )
    goto failure;
  br0cr2 = &br0cr1[hole_k*G2H_OMCDEG];

  if ( !_g2h_GetABasisAuxpd ( domain, fn, bbr0, br0cr1, br0cr2 ) )
    goto failure;
                              ...
                              double *fcomcd, double *fcomcdd )
{
  ...
  int      hole_k;
  double   *bez, *bezbc;
  double   *fcomcbc, *fcomcbcd, *fcomcbcdd;
  int      i, j;
  double   *hole_knots;
  ...
  fcomcbc = pkv_GetScratchMemd ( 3*G2H_OMCDEG*hole_k );
  bez     = pkv_GetScratchMemd ( 24*hole_k );
  bezbc   = pkv_GetScratchMemd ( 36*hole_k );
  ...
  memset ( fcomcbc, 0, 3*G2H_OMCDEG*hole_k*sizeof(double) );

        /* setup the nonzero boundary conditions */
  for ( i = 0; i < hole_k; i++ ) {
    for ( j = 0; j < 2; j++ ) {
      gh_GetDomSurrndBFuncd ( domain, fn, i, j+1, &bezfc[(2*i+j)*16] );
      if ( !mbs_multiBCHornerDer2d ( 3, 1, 4, 0, &bezfc[(2*i+j)*16], 0.0,
              &bez[(2*i+j)*12], &bez[(2*i+j)*12+4], &bez[(2*i+j)*12]+8 ) )
        goto failure;
      memcpy ( &bezfc[(2*i+j)*16+4], &bez[(2*i+j)*12+4], 8*sizeof(double) );
      if ( !mbs_multiBCHornerDer2d ( 3, 3, 1, 4, &bez[(2*i+j)*12], 0.0,
              &bezbc[(2*i+j)*18], &bezbc[(2*i+j)*18+3], &bezbc[(2*i+j)*18+6] ) )
        goto failure;
      if ( !mbs_multiBCHornerDer2d ( 3, 3, 1, 4, &bez[(2*i+j)*12], 1.0,
...
static boolean GetBBasisFunctiond ( GHoleDomaind *domain, int fn,
                                    double *bbr0, double *bbr1, double *bbq1,
                                    double *bbr0cr1, double *bbq0cr1,
                                    double *bbr1cr1, double *bbq1cr1,
                                    double *bbr0cr2, double *bbq0cr2,
                                    double *bbr1cr2, double *bbq1cr2 )
{
  void   *sp;
  G2HolePrivateRecd *privateG2;
  int    hole_k;
  int    i, j;
  double *bezfc, *fcomc, *fcomcd, *fcomcdd;
  double *b01, *c01, *f01, *g01, *b11, *c11, *f11, *g11,
         *b02, *c02, *f02, *g02, *b12, *c12, *f12, *g12,
         *b01b01, *twob01c01, *c01c01, *f01f01, *twof01g01, *g01g01,
  ...
  fcomc = pkv_GetScratchMemd ( 3*G2H_OMCDEG*hole_k );
  if ( !bezfc || !fcomc )
    goto failure;
  fcomcd = &fcomc[(G2H_OMCDEG+1)*hole_k];
  fcomcdd = &fcomcd[G2H_OMCDEG*hole_k];

  if ( !_g2h_GetBBasisAuxpd ( domain, fn, bezfc, fcomc, fcomcd, fcomcdd ) )
    goto failure;
  ...
  for ( i = 0; i < hole_k; i++ ) {
    ...
    memcpy ( &bbr0[i*(G2_CROSS00DEG+1)], &fcomc[i*(G2_CROSS00DEG+1)],
             (G2_CROSS00DEG+1)*sizeof(double) );
    memcpy ( &bbr1[i*(G2_CROSS10DEG+1)], &bezfc[32*j],
             (G2_CROSS10DEG+1)*sizeof(double) );
...
static boolean FindBasisFunctionsd ( GHoleDomaind *domain )
{
  void   *sp;
  G2HolePrivateRecd *privateG2;
  int    hole_k;
  int    i, k, n, nfunc_a, nfunc_b;
  double *bbr0,              *bbr1,    *bbq1,
...
default:
  ...
  privateG2->basis_a = malloc ( nfunc_a*hole_k*n*sizeof(double) );
  privateG2->AuxiMat = malloc ( hole_k*125*sizeof(double) );
  ...
  for ( i = k = 0;  i < nfunc_a;  i++, k += hole_k )
    if ( !GetABasisFunctiond ( domain, i,
  ...
  privateG2->basis_b = malloc ( i = nfunc_b*hole_k*n*sizeof(double) );
  ...
  for ( i = k = 0;  i < nfunc_b;  i++, k += hole_k )
    if ( !GetBBasisFunctiond ( domain, i, &bbr0[k*(G2_CROSS00DEG+1)],
