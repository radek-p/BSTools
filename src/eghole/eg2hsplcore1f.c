...
} /*g2h_DestroySPrivateDataf*/

static boolean g2h_AllocSPrivateDataf ( GHoleDomainf *domain, int nk, int m1, int m2 )
{
  G2HoleSPrivateRecf *sprivate;

  g2h_DestroySPrivateDataf ( domain );
  sprivate = domain->SprivateG2 = malloc ( sizeof(G2HoleSPrivateRecf) );
  if ( !sprivate )
    return false;
  sprivate->nk = nk;
  sprivate->m1 = m1;
  ...
  return true;
} /*g2h_AllocSPrivateDataf*/

static boolean _g2h_ConstructSplKnotSequencesf ( GHoleDomainf *domain )
{
  G2HoleSPrivateRecf *sprivate;
  int    nk, m1, m2, m3, m4, m5;
  int    i, j, k0, k1, k2, k3, k4;
  float  *knr, *knpv, *knpvv, *knb, *cknots, *fpknots;
  float  kn;
  ...
  knr = sprivate->omcknots = malloc ( (sprivate->lastomcknot+1)*sizeof(float) );
  knpv = sprivate->pvknots = malloc ( (sprivate->lastpvknot+1)*sizeof(float) );
  knpvv = sprivate->pvvknots = malloc ( (sprivate->lastpvvknot+1)*sizeof(float) );
  cknots = sprivate->cknots = malloc ( (sprivate->lastcknot+1)*sizeof(float) );
  fpknots = sprivate->fpknots = malloc ( (sprivate->lastfpknot+1)*sizeof(float) );
  ...
  kn = 0.0;
  mbs_SetKnotPatternf ( 0, &kn, G2H_FINALDEG+1, &j, knb );
  kn = 1.0;
  mbs_SetKnotPatternf ( 0, &kn, G2H_FINALDEG+1, &j, &knb[G2H_FINALDEG+1] );
  ...
  kn = 0.0;
  mbs_SetKnotPatternf ( 0, &kn, (k0 = G2_CROSS00DEG+1), &j, knr );
  mbs_SetKnotPatternf ( 0, &kn, (k1 = G2_CROSS01DEG+1), &j, knpv );
  mbs_SetKnotPatternf ( 0, &kn, (k2 = G2_CROSS02DEG+1), &j, knpvv );
  mbs_SetKnotPatternf ( 0, &kn, (k3 = G2H_FINALDEG+1), &j, cknots );
  mbs_SetKnotPatternf ( 0, &kn, (k4 = G2H_FINALDEG+1), &j, fpknots );
  for ( i = 1; i <= nk; i++ ) {
    kn = (float)i/(float)(nk+1);
    mbs_SetKnotPatternf ( 0, &kn, m1, &j, &knr[k0] );      k0 += m1;
    mbs_SetKnotPatternf ( 0, &kn, m3, &j, &knpv[k1] );     k1 += m3;
    mbs_SetKnotPatternf ( 0, &kn, m4, &j, &knpvv[k2] );    k2 += m4;
    mbs_SetKnotPatternf ( 0, &kn, m2, &j, &cknots[k3] );   k3 += m2;
    mbs_SetKnotPatternf ( 0, &kn, m5, &j, &fpknots[k4] );  k4 += m5;
  }
  kn = 1.0;
  mbs_SetKnotPatternf ( 0, &kn, G2_CROSS00DEG+1, &j, &knr[k0] );
  mbs_SetKnotPatternf ( 0, &kn, G2_CROSS01DEG+1, &j, &knpv[k1] );
  mbs_SetKnotPatternf ( 0, &kn, G2_CROSS02DEG+1, &j, &knpvv[k2] );
  mbs_SetKnotPatternf ( 0, &kn, G2H_FINALDEG+1, &j, &cknots[k3] );
  mbs_SetKnotPatternf ( 0, &kn, G2H_FINALDEG+1, &j, &fpknots[k4] );

  return true;
} /*_g2h_ConstructSplKnotSequencesf*/

static boolean _g2h_FindSplCrossDerDf ( const float *knb,
              const float *b1, const float *c1,
              const float *b2, const float *c2,
              const float *b1b1, const float *twob1c1, const float *c1c1,
              int nzc, int ku, int lknr, const float *knr,
              const float *r0, const float *r0s, const float *r0ss,
              int degpv, int lknpv, const float *knpv, float *pv,
              int degpvv, int lknpvv, const float *knpvv, float *pvv )
{
...
/* degree G2H_OMCDEG-1, r0ss, r0us, r0uu are of degree G2H_OMCDEG-2, */
  ...
  void  *sp;
  float *r0u, *r0uu, *r0us;
  float *aux1, *aux2, *aux3, *kn1, *kn2, *kn3;
  int   size, deg1, lkn1, deg2, lkn2, deg3, lkn3;

  sp = pkv_GetScratchMemTop ();
  r0u = pkv_GetScratchMemf ( 3*(lknr-G2H_OMCDEG)-5 );
  size = lknr+1+ku*(G2H_FINALDEG-G2H_OMCDEG+2);
  aux1 = pkv_GetScratchMemf ( 6*size );
  if ( !r0u || !aux1 )
    goto failure;
...
case 0:   /* nonzero is r0 and its derivatives, r0u, r0uu */
    mbs_FindBSDerivativeC1f ( G2H_OMCDEG, lknr, knr, r0, NULL, NULL, r0u );
    mbs_FindBSDerivativeC1f ( G2H_OMCDEG-1, lknr-2, &knr[1], r0u, NULL, NULL, r0uu );
    mbs_FindBSDerivativeC1f ( G2H_OMCDEG-1, lknr-2, &knr[1], r0s, NULL, NULL, r0us );

      /* compute pv = b1*r0u + c1*r0s = b1*r0u */
    lkn1 = size;
    mbs_multiMultBSCf ( 1, G2_BF01DEG, 2*G2_BF01DEG+1, &knb[G2H_FINALDEG-G2_BF01DEG],
                        0, b1, 1, 1, G2H_OMCDEG-1, lknr-2, &knr[1], 0, r0u,
                        &deg1, &lkn1, kn1, 0, aux1 );
    mbs_multiAdjustBSCRepf ( 1, 1, deg1, lkn1, kn1, 0, aux1,
                             degpv, lknpv, knpv, 0, pv );

      /* compute pvv = b2*r0u + c2*r0s + b1b1*r0uu + twob1c1*r0us + c1c1*r0ss */
      /* = b2*r0u + b1b1*r0uu */
    lkn1 = size;
    mbs_multiMultBSCf ( 1, G2_BF02DEG, 2*G2_BF02DEG+1, &knb[G2H_FINALDEG-G2_BF02DEG],
                        0, b2, 1, 1, G2H_OMCDEG-1, lknr-2, &knr[1], 0, r0u,
                        &deg1, &lkn1, kn1, 0, aux1 );
    lkn2 = size;
    mbs_multiMultBSCf ( 1, 2*G2_BF01DEG, 4*G2_BF01DEG+1, &knb[G2H_FINALDEG-2*G2_BF01DEG],
...
case 1:   /* nonzero is r0s and its derivative, r0us */
    ...
    lkn1 = size;
    mbs_multiMultBSCf ( 1, G2_CG01DEG, 2*G2_CG01DEG+1, &knb[G2H_FINALDEG-G2_CG01DEG],
    ...
    mbs_multiAdjustBSCRepf ( 1, 1, deg1, lkn1, kn1, 0, aux1,
      ...
      /* = c2*r0s + twob1c1*r0us */
    lkn1 = size;
    mbs_multiMultBSCf ( 1, G2_CG02DEG, 2*G2_CG02DEG+1, &knb[G2H_FINALDEG-G2_CG02DEG],
                        0, c2, 1, 1, G2H_OMCDEG-1, lknr-2, &knr[1], 0, r0s,
                        &deg1, &lkn1, kn1, 0, aux1 );
    lkn2 = size;
    mbs_multiMultBSCf ( 1, G2_BF01DEG+G2_CG01DEG, 2*(G2_BF01DEG+G2_CG01DEG)+1,
...
case 2:   /* nonzero is r0ss */
    ...
    memset ( pv, 0, (lknpv-degpv)*sizeof(float) );

      /* compute pvv = b2*r0u + c2*r0s + b1b1*r0uu + twob1c1*r0us + c1c1*r0ss */
      /* = c1c1*r0ss */
    lkn1 = size;
    mbs_multiMultBSCf ( 1, 2*G2_CG01DEG, 4*G2_CG01DEG+1, &knb[G2H_FINALDEG-2*G2_CG01DEG],
                   ...
                   int *nzc, float *fcomc, float *fcomcd, float *fcomcdd )
{
  ...
  memset ( fcomc, 0, (G2H_OMCDEG+1+nk*m1)*sizeof(float) );
  memset ( fcomcd, 0, (G2H_OMCDEG+nk*m1)*sizeof(float) );
  memset ( fcomcdd, 0, (G2H_OMCDEG-1+nk*m1)*sizeof(float) );
...
/* the i-th in the arrays pv, pvv and (i-1) mod hole_k in the arrays pu, puu. */
  ...
  memset ( fcomc, 0, (lpkn-G2H_OMCDEG)*sizeof(float) );
  if ( cn == fn/(3*nk*m1) )  {
    i = fn % (3*nk*m1);
    if ( (i % 3) == 0 ) fcomc[i/3+5] = 1.0;
    basis_d = sprivate->basis_d;
    j = fn*2*(lpvkn+lpvvkn-G2_CROSS01DEG-G2_CROSS02DEG);
    memcpy ( pv, &basis_d[j], (lpvkn-G2_CROSS01DEG)*sizeof(float) );
    j += lpvkn-G2_CROSS01DEG;
    memcpy ( pvv, &basis_d[j], (lpvvkn-G2_CROSS02DEG)*sizeof(float) );
    j += lpvvkn-G2_CROSS02DEG;
    memcpy ( pu, &basis_d[j], (lpvkn-G2_CROSS01DEG)*sizeof(float) );
    j += lpvkn-G2_CROSS01DEG;
    memcpy ( puu, &basis_d[j], (lpvvkn-G2_CROSS02DEG)*sizeof(float) );
  }
  else {
    memset ( pv,  0, (lpvkn-G2_CROSS01DEG)*sizeof(float) );
    memset ( pvv, 0, (lpvvkn-G2_CROSS02DEG)*sizeof(float) );
    memset ( pu,  0, (lpvkn-G2_CROSS01DEG)*sizeof(float) );
    memset ( puu, 0, (lpvvkn-G2_CROSS02DEG)*sizeof(float) );
  }

  return true;
} /*_g2h_GetSplDBasisCrossDerf*/

static boolean _g2h_FindSplBasisFunctionsDf ( GHoleDomainf *domain )
{
  void  *sp;
  G2HolePrivateRecf  *privateG2;
  G2HoleSPrivateRecf *sprivate;
  ...
  float *knpv, *knpvv, *knb, *knr, *fcomc, *fcomcd, *fcomcdd, *basis_d;
  ...
  basis_d = sprivate->basis_d = malloc ( nfunc_d*2*
      (lpvkn+lpvvkn-G2_CROSS01DEG-G2_CROSS02DEG)*sizeof(float) );
  if ( !basis_d )
    goto failure;
        /* allocate memory for the spline auxiliary basis function patches */
  fcomc = pkv_GetScratchMemf ( 3*(G2H_OMCDEG+nk*m1) );
  if ( !fcomc )
    goto failure;
...
failure:
  pkv_SetScratchMemTop ( sp );
  return false;
} /*_g2h_FindSplBasisFunctionsDf*/

static boolean _g2h_FindSplBasisFunctionsf ( GHoleDomainf *domain )
{
  void               *sp;
  G2HoleSPrivateRecf *sprivate;
  int                nk, m1, m2, hole_k;
  ...
  if ( !_g2h_ConstructSplKnotSequencesf ( domain ) )
    goto failure;
        /* find the basis function patches for the block D */
  if ( !_g2h_FindSplBasisFunctionsDf ( domain ) )
    goto failure;
...
boolean g2h_ComputeSplBasisf ( GHoleDomainf *domain, int nk, int m1, int m2 )
{
  ...
  if ( !g2h_AllocSPrivateDataf ( domain, nk, m1, m2 ) )
    return false;
  ...
  if ( !g2h_ComputeBasisf ( domain ) )
    return false;
  if ( !_g2h_FindSplBasisFunctionsf ( domain ) )
    return false;
  return true;
} /*g2h_ComputeSplBasisf*/
