...
static boolean AnalyzePartitiond ( GHoleDomaind *domain )
{
  ...
  if ( _gh_AnalyzePartitiond ( domain, G1H_OMCDEG, privateG1->omcbc,
...
static void ComputeAuxiMat ( GHoleDomaind *domain )
{
  G1HolePrivateRecd *privateG1;
  int      i, hole_k;
  vector2d diu, div, diuu, diuv;
  vector2d *omcbc, *omcbcd;
  double   *A11, *A21, *A22;
...
#define TOL 1.0e-6
  ...
  br0bc = pkv_GetScratchMemd ( (2*G1H_OMCDEG+1)*hole_k );
  if ( !br0bc )
    goto failure;

  br0cr1bc = &br0bc[hole_k*(G1H_OMCDEG+1)];

  memset ( br0bc, 0, hole_k*3*G1_CROSS00DEG*sizeof(double) );
  switch ( fn ) {
case 0:    /* f(x,y) = 1 */
    for ( i = 0; i < hole_k; i++ )
...
default:   /* splines */
    ...
    if ( sfn < spdimen[0] ) {                 /* a quadratic half-polynomial */
      ...
      p2[0] = (double)(-2.0*sa*sa);  p2[1] = (double)(2.0*sa*ca);
      p2[2] = (double)(-2.0*ca*ca);
      for ( i = 0; i < hole_k; i++ ) {
      ...
      for ( j = 0; j < 4; j++ ) {
        ...
        p2[0] = (double)(2.0*ddc[j]*a*a);
        p2[1] = (double)(2.0*ddc[j]*a*b);
        p2[2] = (double)(2.0*ddc[j]*b*b);
...
static boolean _g1h_GetABasisFunctiond ( GHoleDomaind *domain, int fn,
                                         double *bbr0,
...
#define TOL 1.0e-6
  ...
  br0cr1 = pkv_GetScratchMemd ( hole_k*G1H_OMCDEG );
  if ( !br0cr1 )
    goto failure;

  if ( !_g1h_GetABasisAuxpd ( domain, fn, bbr0, br0cr1 ) )
    goto failure;
  ...
  for ( i = 0; i < hole_k; i++ ) {
    j = (i+1) % hole_k;
    FindDiCrossDerad ( 1, &b01[i*(G1_BF01DEG+1)], &c01[i*(G1_CG01DEG+1)],
        &bbr0[i*(G1H_OMCDEG+1)], &br0cr1[i*G1H_OMCDEG], &bbr0cr1[i*(G1_CROSS01DEG+1)] );
    FindDiCrossDerad ( 1, &f01[i*(G1_BF01DEG+1)], &g01[i*(G1_CG01DEG+1)],
                              ...
                              double *bezfc, double *fcomc, double *fcomcd )
{
  ...
  fcomcbc = pkv_GetScratchMemd ( (2*G1H_OMCDEG+1)*hole_k );
  ...
  memset ( fcomcbc, 0, (2*G1H_OMCDEG+1)*hole_k*sizeof(double) );
                                    ...
                                    double *bbr1cr1, double *bbq1cr1 )
{
  ...
  fcomc = pkv_GetScratchMemd ( (2*G1H_OMCDEG+1)*hole_k );
  if ( !bezfc || !fcomc )
    goto failure;
  fcomcd = &fcomc[(G1H_OMCDEG+1)*hole_k];

  if ( !_g1h_GetBBasisAuxpd ( domain, fn, bezfc, fcomc, fcomcd ) )
    goto failure;
  ...
  for ( i = 0; i < hole_k; i++ ) {
    ...
    memcpy ( &bbr0[i*(G1_CROSS00DEG+1)], &fcomc[i*(G1_CROSS00DEG+1)],
             (G1_CROSS00DEG+1)*sizeof(double) );
    memcpy ( &bbr1[i*(G1_CROSS10DEG+1)], &bezfc[32*j],
             (G1_CROSS10DEG+1)*sizeof(double) );
    ...
    FindDiCrossDerbd ( 1, &b11[i*(G1_BF11DEG+1)], &c11[i*(G1_CG11DEG+1)],
        &bezfc[32*j], &bezfc[32*j+4], &bbr1cr1[i*(G1_CROSS11DEG+1)] );
    FindDiCrossDerbd ( 1, &f11[i*(G1_BF11DEG+1)], &g11[i*(G1_CG11DEG+1)],
...
default:
  ...
  privateG1->basis_a = malloc ( nfunc_a*hole_k*n*sizeof(double) );
  privateG1->AuxiMat = malloc ( hole_k*19*sizeof(double) );
  ...
  for ( i = k = 0;  i < nfunc_a;  i++, k += hole_k )
    if ( !_g1h_GetABasisFunctiond ( domain, i,
  ...
  privateG1->basis_b = malloc ( i = nfunc_b*hole_k*n*sizeof(double) );
  ...
  for ( i = k = 0;  i < nfunc_b;  i++, k += hole_k )
    if ( !GetBBasisFunctiond ( domain, i,
                  &bbr0[k*(G1_CROSS00DEG+1)],
