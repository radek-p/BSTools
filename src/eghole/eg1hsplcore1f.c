...
} /*g1h_DestroySPrivateDataf*/

static boolean g1h_AllocSPrivateDataf ( GHoleDomainf *domain, int nk, int m1, int m2 )
{
  G1HoleSPrivateRecf *sprivate;

  g1h_DestroySPrivateDataf ( domain );
  sprivate = domain->SprivateG1 = malloc ( sizeof(G1HoleSPrivateRecf) );
  if ( !sprivate )
    return false;
  sprivate->nk = nk;
  sprivate->m1 = m1;
  ...
  return true;
} /*g1h_AllocSPrivateDataf*/

static boolean _g1h_ConstructSplKnotSequencesf ( GHoleDomainf *domain )
{
  G1HoleSPrivateRecf *sprivate;
  int    nk, m1, m2, m3, m5;
  int    i, j, k0, k1, k3, k4;
  float  *knr, *knpv, *knb, *cknots, *fpknots;
  float  kn;
  ...
  kn = 0.0;
  mbs_SetKnotPatternf ( 0, &kn, G1H_FINALDEG+1, &j, knb );
  kn = 1.0;
  mbs_SetKnotPatternf ( 0, &kn, G1H_FINALDEG+1, &j, &knb[G1H_FINALDEG+1] );
  ...
  kn = 0.0;
  mbs_SetKnotPatternf ( 0, &kn, (k0 = G1_CROSS00DEG+1), &j, knr );
  mbs_SetKnotPatternf ( 0, &kn, (k1 = G1_CROSS01DEG+1), &j, knpv );
  mbs_SetKnotPatternf ( 0, &kn, (k3 = G1H_FINALDEG+1), &j, cknots );
  mbs_SetKnotPatternf ( 0, &kn, (k4 = G1H_FINALDEG+1), &j, fpknots );
  for ( i = 1; i <= nk; i++ ) {
  ...
  kn = 1.0;
  mbs_SetKnotPatternf ( 0, &kn, G1_CROSS00DEG+1, &j, &knr[k0] );
  mbs_SetKnotPatternf ( 0, &kn, G1_CROSS01DEG+1, &j, &knpv[k1] );
  mbs_SetKnotPatternf ( 0, &kn, G1H_FINALDEG+1, &j, &cknots[k3] );
  mbs_SetKnotPatternf ( 0, &kn, G1H_FINALDEG+1, &j, &fpknots[k4] );

  return true;
} /*_g1h_ConstructSplKnotSequencesf*/

static boolean _g1h_FindSplCrossDerDf ( const float *knb,
...
/* degree G1H_OMCDEG-1. */
  ...
  r0u = pkv_GetScratchMemf ( 2*(lknr-G1H_OMCDEG)-3 );
  size = lknr+1+ku*(G1H_FINALDEG-G1H_OMCDEG+2);
  aux = pkv_GetScratchMemf ( 2*size );
  if ( !r0u || !aux )
    goto failure;
  kn = &aux[size];
...
case 0:   /* nonzero is r0 and its derivative, r0u */
    mbs_FindBSDerivativeC1f ( G1H_OMCDEG, lknr, knr, r0, NULL, NULL, r0u );

      /* compute pv = b1*r0u + c1*r0s = b1*r0u */
    lkn = size;
    mbs_multiMultBSCf ( 1, G1_BF01DEG, 2*G1_BF01DEG+1, &knb[G1H_FINALDEG-G1_BF01DEG],
                        0, b1, 1, 1, G1H_OMCDEG-1, lknr-2, &knr[1], 0, r0u,
                        &deg, &lkn, kn, 0, aux );
    mbs_multiAdjustBSCRepf ( 1, 1, deg, lkn, kn, 0, aux,
...
case 1:   /* nonzero is r0s */
    ...
    lkn = size;
    mbs_multiMultBSCf ( 1, G1_CG01DEG, 2*G1_CG01DEG+1, &knb[G1H_FINALDEG-G1_CG01DEG],
                                 ...
                                 int *nzc, float *fcomc, float *fcomcd )
{
  ...
  memset ( fcomc, 0, (G1H_OMCDEG+1+nk*m1)*sizeof(float) );
  memset ( fcomcd, 0, (G1H_OMCDEG+nk*m1)*sizeof(float) );
...
/* the i-th in the arrays pv, pvv and (i-1) mod hole_k in the arrays pu, puu. */
  ...
  memset ( fcomc, 0, (lpkn-G1H_OMCDEG)*sizeof(float) );
  if ( cn == fn/(2*nk*m1) )  {
    i = fn % (2*nk*m1);
    if ( (i % 2) == 0 ) fcomc[i/2+3] = 1.0;
    basis_d = sprivate->basis_d;
    j = fn*2*(lpvkn-G1_CROSS01DEG);
    memcpy ( pv, &basis_d[j], (lpvkn-G1_CROSS01DEG)*sizeof(float) );
    j += lpvkn-G1_CROSS01DEG;
    memcpy ( pu, &basis_d[j], (lpvkn-G1_CROSS01DEG)*sizeof(float) );
  }
  else {
    memset ( pv,  0, (lpvkn-G1_CROSS01DEG)*sizeof(float) );
    memset ( pu,  0, (lpvkn-G1_CROSS01DEG)*sizeof(float) );
  }

  return true;
} /*_g1h_GetSplDBasisCrossDerf*/

static boolean _g1h_FindSplBasisFunctionsDf ( GHoleDomainf *domain )
{
  void  *sp;
  G1HolePrivateRecf  *privateG1;
  G1HoleSPrivateRecf *sprivate;
  ...
  int   nfunc_a, nfunc_b, nfunc_c, nfunc_d;
  int   lomckn, lpvkn;
  float *knpv, *knb, *knr, *fcomc, *fcomcd, *basis_d;
  ...
  basis_d = sprivate->basis_d = malloc ( nfunc_d*2*
                                         (lpvkn-G1_CROSS01DEG)*sizeof(float) );
  if ( !basis_d )
    goto failure;
        /* allocate memory for the spline auxiliary basis function patches */
  fcomc = pkv_GetScratchMemf ( 2*(G1H_OMCDEG+1+nk*m1) );
  if ( !fcomc )
    goto failure;
...
failure:
  pkv_SetScratchMemTop ( sp );
  return false;
} /*_g1h_FindSplBasisFunctionsDf*/

static boolean _g1h_FindSplBasisFunctionsf ( GHoleDomainf *domain )
{
  void               *sp;
  G1HoleSPrivateRecf *sprivate;
  int                nk, m1, m2, hole_k;
  ...
  if ( !_g1h_ConstructSplKnotSequencesf ( domain ) )
    goto failure;
        /* find the basis function patches for the block D */
  if ( !_g1h_FindSplBasisFunctionsDf ( domain ) )
    goto failure;
...
boolean g1h_ComputeSplBasisf ( GHoleDomainf *domain, int nk, int m1, int m2 )
{
  ...
  if ( !g1h_AllocSPrivateDataf ( domain, nk, m1, m2 ) )
    return false;
  ...
  if ( !g1h_ComputeBasisf ( domain ) )
    return false;
  if ( !_g1h_FindSplBasisFunctionsf ( domain ) )
    return false;
  return true;
} /*g1h_ComputeSplBasisf*/
