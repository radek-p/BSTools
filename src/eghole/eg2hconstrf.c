...
boolean g2h_GetBFuncPatchf ( GHoleDomainf *domain, int fn, int pn, float *bp )
{
  ...
  if ( fn < 0 )
    ...
    _g2h_GetBFAPatchCurvesf ( domain, fn, pn,
    ...
    _g2h_GetBFBPatchCurvesf ( domain, fn-nfunc_a, pn,
                                   ...
                                   int nconstr, const float *cmat )
{
  ...
  Cmat = privateG2->Cmat = malloc ( nfunc_a*nconstr*sizeof(float) );
  RCmat = privateG2->RCmat = malloc ( (nconstr*(nconstr+1))/2*sizeof(float) );
  if ( !Cmat || !RCmat )
    goto failure;
  ...
  pkv_TransposeMatrixf ( nconstr, nfunc_a, nfunc_a, privateG2->Cmat, nconstr, Cmat );
  pkn_LowerTrMatrixSolvef ( nfunc_a, privateG2->Lmat,
                                      ...
                                      int naconstr, const float *acmat )
{
  ...
  ACmat = privateG2->ACmat = malloc ( nfunc_a*spdimen*naconstr*sizeof(float) );
  ARCmat = privateG2->ARCmat = malloc ( (naconstr*(naconstr+1))/2*sizeof(float) );
  if ( !ACmat || !ARCmat )
    goto failure;
