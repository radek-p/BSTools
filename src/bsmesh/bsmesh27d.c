...
/* This procedure uses the same averaging algorithm that the bsm_Averagingd */
/* procedure, which is intended to obtain the same correspondence between   */
/* the vertices of the input and output mesh - the only difference is that  */
/* it produces the averaging matrix instead of the resulting vertices.      */
/* Should any modifications be necessary, te same have to be made in the    */
/* code of bsm_Averagingd.                                                  */
...
{
  void   *sp;
  int    vi, vb, ei, eb, _onv, _onhe, _onfac, fvn, _namat;
  ...
  for ( i = 0;  i < inv;  i++ ) {
    ...
    if ( k >= 0 ) {
      ...
      for ( m = d-1, v1 = omhe[omfhei[l]].v0;
        ...
        /* generating entries of the averaging matrix */
  _namat = 0;
  for ( i = 0; i < infac; i++ )
    if ( fvnum[i] > 0 ) {
      n = nvi[i];
      ...
      for ( k = 0; k < d; k++ )
        for ( s = 0; s < fvnum[i]; s++ ) {
          ami[_namat].i = n+s;
          ami[_namat].j = imhe[imfhei[j+k]].v0;
          amc[_namat++] = ac;
        }
    }
  *namat = _namat;
