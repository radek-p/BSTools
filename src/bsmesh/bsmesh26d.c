...
/* This procedure uses the same doubling algorithm that the bsm_Doublingd */
...
/* that it produces the doubling matrix instead of copies of the input    */
/* vertices. Should any modifications be necessary, the same have to be   */
/* made in the code of bsm_Doublingd.                                     */
...
#define PREVIFAC_HEDGE(fn,en) \
  ...
  int  *vcn, *ecn, *efn;
  char *wlv, *wlf;
  int  i, j, k, l, m, f, d, v0, v1, p, ecni, ecnl, _ndmat;
  char *vtag, *ftag;
  ...
  for ( i = 0; i < inhe; i++ )
    if ( imhe[i].otherhalf == -1 )
      ...
      ecn[i+1] = ecn[i] + 4;
        ...
        /* then enter the doubling matrix coefficient */
  _ndmat = 0;
  vcn[0] = 0;
  for ( i = 0; i < inv; i++ ) {
    if ( vtag[i] )
